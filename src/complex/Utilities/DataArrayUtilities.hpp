...
#if 0
struct CopyTupleUsingIndexListFunctor
{
  template <class T>
...
public:
  ...
  void convert(size_t start, size_t end) const
  {
    ...
    {
      int64 nexIndex = m_NewIndices[i];
      if(nexIndex >= 0)
      {
        if(!newDataStore.copyFrom(i, oldDataStore, nexIndex, 1))
        {
          std::cout << fmt::format("Array copy failed: Source Array Name: {} Source Tuple Index: {}\nDest Array Name: {}  Dest. Tuple Index {}\n", m_OldCellArray.getName(), nexIndex, i) << std::endl;
          break;
        }
      }
      else
