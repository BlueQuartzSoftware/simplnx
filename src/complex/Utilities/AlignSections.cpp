...
public:
  ...
  AlignSectionsTransferDataImpl(AlignSections* filter, SizeVec3 dims,
                                const std::vector<int64_t>& xShifts,
                                const std::vector<int64_t>& yShifts,
                                 complex::DataArray<T>& dataArray)
  : m_Filter(filter)
  , m_Dims(dims)
  , m_Xshifts(xShifts)
  , m_Yshifts(yShifts)
...
void AlignSections::updateProgress(size_t p)
{
  ...
  std::string ss = fmt::format("Transferring Cell Data {}%", progressInt);
...
Result<> AlignSections::execute(complex::AbstractGeometryGrid* gridGeom)
{
  ...
  m_TotalProgress = udims[0] * udims[1] * udims[2];
