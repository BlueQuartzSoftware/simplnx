...
{
  m_PolyhedraListId = ReadH5DataId(groupReader, H5Constants::k_HexListTag);
  m_ElementContainingVertId = ReadH5DataId(groupReader, H5Constants::k_HexasContainingVertTag);
  m_ElementNeighborsId = ReadH5DataId(groupReader, H5Constants::k_HexNeighborsTag);
  m_ElementCentroidsId = ReadH5DataId(groupReader, H5Constants::k_HexCentroidsTag);
  m_ElementSizesId = ReadH5DataId(groupReader, H5Constants::k_HexSizesTag);
...
H5::ErrorType HexahedralGeom::writeHdf5(H5::DataStructureWriter& dataStructureWriter, H5::GroupWriter& parentGroupWriter, bool importable) const
{
  ...
  errorCode = WriteH5DataId(groupWriter, m_PolyhedraListId, H5Constants::k_HexListTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementContainingVertId, H5Constants::k_HexasContainingVertTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementNeighborsId, H5Constants::k_HexNeighborsTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementCentroidsId, H5Constants::k_HexCentroidsTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementSizesId, H5Constants::k_HexSizesTag);
  if(errorCode < 0)
  {
    return errorCode;
  }
