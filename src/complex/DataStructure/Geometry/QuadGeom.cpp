...
{
  m_FaceListId = ReadH5DataId(groupReader, H5Constants::k_QuadListTag);
  m_ElementContainingVertId = ReadH5DataId(groupReader, H5Constants::k_QuadsContainingVertTag);
  m_ElementNeighborsId = ReadH5DataId(groupReader, H5Constants::k_QuadNeighborsTag);
  m_ElementCentroidsId = ReadH5DataId(groupReader, H5Constants::k_QuadCentroidsTag);
  m_ElementSizesId = ReadH5DataId(groupReader, H5Constants::k_QuadSizesTag);
...
H5::ErrorType QuadGeom::writeHdf5(H5::DataStructureWriter& dataStructureWriter, H5::GroupWriter& parentGroupWriter, bool importable) const
{
  ...
  errorCode = WriteH5DataId(groupWriter, m_FaceListId, H5Constants::k_QuadListTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementContainingVertId, H5Constants::k_QuadsContainingVertTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementNeighborsId, H5Constants::k_QuadNeighborsTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementCentroidsId, H5Constants::k_QuadCentroidsTag);
  if(errorCode < 0)
  {
    return errorCode;
  }

  errorCode = WriteH5DataId(groupWriter, m_ElementSizesId, H5Constants::k_QuadSizesTag);
  if(errorCode < 0)
  {
    return errorCode;
  }
