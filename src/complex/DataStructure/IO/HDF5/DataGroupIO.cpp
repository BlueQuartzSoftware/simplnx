...
Result<> DataGroupIO::writeData(DataStructureWriter& dataStructureWriter, const DataGroup& dataGroup, group_writer_type& parentGroup, bool importable) const
{
  ...
  Result<> result = WriteObjectAttributes(dataStructureWriter, dataGroup, groupWriter, importable);
  if(result.invalid())
  {
    return result;
  }

  return WriteDataMap(dataStructureWriter, dataGroup.getDataMap(), groupWriter, importable);
}
