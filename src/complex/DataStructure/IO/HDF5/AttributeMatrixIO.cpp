...
Result<> AttributeMatrixIO::writeData(DataStructureWriter& dataStructureWriter, const data_type& attributeMatrix, group_writer_type& parentGroup, bool importable) const
{
  ...
  Result<> result = WriteObjectAttributes(dataStructureWriter, attributeMatrix, groupWriter, importable);
  if(result.invalid())
  {
    return result;
  }
  ...
  auto error = tupleDimsAttribute.writeVector(H5::AttributeWriter::DimsVector{tupleShape.size()}, tupleShape);
