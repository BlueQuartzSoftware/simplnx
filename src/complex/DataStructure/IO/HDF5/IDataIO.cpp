...
Result<> IDataIO::WriteObjectAttributes(DataStructureWriter& dataStructureWriter, const DataObject& dataObject, object_writer_type& objectWriter, bool importable)
{
  ...
  if(error < 0)
  {
    std::string ss = fmt::format("Error writing DataObject attribute: {}", complex::Constants::k_ObjectTypeTag);
  ...
  if(error < 0)
  {
    std::string ss = fmt::format("Error writing DataObject attribute: {}", complex::Constants::k_ObjectIdTag);
    return MakeErrorResult(error, ss);
  }
  ...
  if(error < 0)
  {
    std::string ss = fmt::format("Error writing DataObject attribute: {}", complex::Constants::k_ImportableTag);
    return MakeErrorResult(error, ss);
  }
