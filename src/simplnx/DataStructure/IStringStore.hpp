...
public:
  ...
  public:
    ...
    using value_type = IStringStore::value_type;
    ...
    Iterator(IStringStore& dataStore, usize index)
  ...
  public:
    ...
    ConstIterator(const IStringStore& dataStore, usize index)
    : m_DataStore(&dataStore)
    , m_Index(index)
    {
    }
  ...
  private:
    const IStringStore* m_DataStore = nullptr;
    usize m_Index = 0;
  };
  ...
  virtual std::unique_ptr<IStringStore> deepCopy() const = 0;
  ...
  virtual IStringStore& operator=(const std::vector<std::string>& values) = 0;
