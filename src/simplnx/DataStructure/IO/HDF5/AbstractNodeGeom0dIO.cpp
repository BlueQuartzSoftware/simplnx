#include "AbstractNodeGeom0dIO.hpp"
...
{
AbstractNodeGeom0dIO::AbstractNodeGeom0dIO() = default;
AbstractNodeGeom0dIO::~AbstractNodeGeom0dIO() noexcept = default;

Result<> AbstractNodeGeom0dIO::ReadNodeGeom0dData(DataStructureReader& dataStructureReader, AbstractNodeGeometry0D& geometry, const group_reader_type& parentGroup, const std::string& objectName,
...
Result<> AbstractNodeGeom0dIO::FinishImportingNodeGeom0dData(DataStructure& dataStructure, const DataPath& dataPath, const group_reader_type& dataStructureGroup)
{
  auto* geom = dataStructure.getDataAs<AbstractNodeGeometry0D>(dataPath);
  if(geom == nullptr)
  {
    return MakeErrorResult(-50590, fmt::format("Failed to finish importing AbstractNodeGeometry0D at path '{}'. Data not found or of incorrect type.", dataPath.toString()));
  }
...
Result<> AbstractNodeGeom0dIO::WriteNodeGeom0dData(DataStructureWriter& dataStructureWriter, const AbstractNodeGeometry0D& geometry, group_writer_type& parentGroupWriter, bool importable)
{
  Result<> result = AbstractGeometryIO::WriteGeometryData(dataStructureWriter, geometry, parentGroupWriter, importable);
  if(result.invalid())
  {
    return result;
  }
  ...
  AbstractDataObject::OptionalId vertexListId = geometry.getVertexListId();
