 ...
 * @brief The AbstractDataArray class is a typeless interface for the templated DataArray class.
 */
class SIMPLNX_EXPORT AbstractDataArray : public AbstractArray
{
public:
  static constexpr StringLiteral k_TypeName = "IDataArray";
  ...
  IDataStore& getIDataStoreRef()
  {
    ...
    if(store == nullptr)
    {
      throw std::runtime_error("AbstractDataArray: Null IDataStore");
    }
    return *store;
  }
...
protected:
  AbstractDataArray(DataStructure& dataStructure, std::string name)
  : AbstractArray(dataStructure, std::move(name))
  {
  }
  AbstractDataArray(DataStructure& dataStructure, std::string name, IdType importId)
