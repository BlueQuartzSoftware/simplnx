...
public:
  ...
  Result<std::vector<T>> readVectorAttribute(const std::string& attributeName) const
  {
    ...
    {
      std::lock_guard<std::mutex> hdf5Lock(Support::ApiLock());
      const hid_t fileTypeId = H5Aget_type(attribId);
      if(fileTypeId >= 0)
      {
        const H5T_class_t typeClass = H5Tget_class(fileTypeId);
        compatibleType = typeClass == H5T_INTEGER || typeClass == H5T_FLOAT || typeClass == H5T_ENUM;
        H5Tclose(fileTypeId);
      }
      if(compatibleType && !values.empty())
      {
        const hid_t memoryTypeId = HdfTypeForPrimitive<T>();
        error = memoryTypeId < 0 ? -1 : H5Aread(attribId, memoryTypeId, values.data());
      }
      H5Aclose(attribId);
    }
    if(!compatibleType)
    {
      return MakeErrorResult<std::vector<T>>(-102, fmt::format("Attribute '{}' within '{}' is not numeric.", attributeName, getName()));
    }
    if(error < 0)
    {
