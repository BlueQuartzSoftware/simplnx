...
public:
  ...
  enum
  {
    MAXNODES = TMAXNODES, ///< RTREE_MAX elements in node
    MINNODES = TMINNODES, ///< RTREE_MIN elements in node
  };
...
public:
  ...
  /// Insert entry
  /// \param a_min RTREE_MIN of bounding rect
  /// \param a_max RTREE_MAX of bounding rect
  ...
  /// \param a_min RTREE_MIN of search bounding rect
  /// \param a_max RTREE_MAX of search bounding rect
  ...
  private:
    ...
    {
      MAX_STACK = 32
    }; //  RTREE_MAX stack size. Allows almost n^32 where n is number of branches in node
...
protected:
  ...
  struct Rect
  {
    ELEMTYPE m_min[NUMDIMS]; ///< RTREE_MIN dimensions of bounding box
    ELEMTYPE m_max[NUMDIMS]; ///< RTREE_MAX dimensions of bounding box
  };
...
void RTREE_QUAL::Reset()
{
  ...
  RemoveAllRec(m_root);
#else
  // Just reset memory pools.  We are not using simplnx types
  // EXAMPLE
#endif
}
