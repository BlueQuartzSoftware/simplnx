...
//   Flatten the Union-Find tree, then scan the featureIds store slice by
//   slice in Z-Y-X order. For each provisional label, look up its Union-Find
//   root and map it to a contiguous final feature ID. Write the final ID
//   back in the same pass. This combined discover-and-write approach halves
//   the number of OOC accesses compared to separate resolution and write
//   passes, and slice-sequential iteration ensures optimal I/O.
// =============================================================================
Result<> SegmentFeatures::executeCCL(IGridGeometry* gridGeom, AbstractDataStore<int32>& featureIdsStore)
{
  ...
  for(int64 iz = 0; iz < dimZ; iz++)
  {
    ...
    // Clear the featureIds slice buffer for this Z-slice
    std::fill(featureIdsSlice.begin(), featureIdsSlice.end(), 0);
    ...
    featureIdsStore.copyFromBuffer(static_cast<usize>(iz) * sliceSize, nonstd::span<const int32>(featureIdsSlice.data(), sliceSize));
  ...
  if(m_IsPeriodic)
  {
    ...
    if(useFaceOnly)
    {
      ...
      if(dimX > 1)
      {
        for(int64 iz = 0; iz < dimZ; iz++)
        {
          featureIdsStore.copyIntoBuffer(static_cast<usize>(iz) * sliceSize, nonstd::span<int32>(featureIdsSliceCur.data(), sliceSize));
          prepareForSlice(iz, dimX, dimY, dimZ);
      ...
      if(dimZ > 1)
      {
        ...
        featureIdsStore.copyIntoBuffer(0, nonstd::span<int32>(featureIdsSliceCur.data(), sliceSize));
        featureIdsStore.copyIntoBuffer(static_cast<usize>(dimZ - 1) * sliceSize, nonstd::span<int32>(featureIdsSliceOther.data(), sliceSize));
        ...
        prepareForSlice(0, dimX, dimY, dimZ);
        prepareForSlice(dimZ - 1, dimX, dimY, dimZ);
    ...
    else
    {
      ...
      featureIdsStore.copyIntoBuffer(0, nonstd::span<int32>(featureIdsSlice0.data(), sliceSize));
      if(dimZ > 1)
      {
        featureIdsStore.copyIntoBuffer(static_cast<usize>(dimZ - 1) * sliceSize, nonstd::span<int32>(featureIdsSliceLast.data(), sliceSize));
      }
      ...
      for(int64 iz = 0; iz < dimZ; iz++)
      {
        ...
        if(iz == 0 && dimZ > 1)
        {
          ...
          prepareForSlice(wrappedPartnerZ, dimX, dimY, dimZ);
        }
  ...
  //   2. Scans voxels slice-by-slice in Z-Y-X order. For each
  ...
  for(int64 iz = 0; iz < dimZ; iz++)
  {
    ...
    featureIdsStore.copyIntoBuffer(static_cast<usize>(iz) * sliceSize, nonstd::span<int32>(sliceData.data(), sliceSize));
    ...
    for(int64 iy = 0; iy < dimY; iy++)
    {
      for(int64 ix = 0; ix < dimX; ix++)
      {
        ...
        if(label > 0)
        {
          ...
          // Write the final contiguous feature ID back to the slice buffer
          sliceData[inSlice] = labelToFinal[label];
        }
      }
    }

    featureIdsStore.copyFromBuffer(static_cast<usize>(iz) * sliceSize, nonstd::span<const int32>(sliceData.data(), sliceSize));

    // Send progress
    float percentComplete = static_cast<float>(iz + 1) / static_cast<float>(dimZ) * 100.0f;
    throttledMessenger.sendThrottledMessage([percentComplete]() { return fmt::format("Relabeling: {:.1f}% complete", percentComplete); });
  }
