...
The file is opened in preflight mode: only object names, types, shapes and
geometry metadata are read. Array values are never loaded, so a dump of a
multi-gigabyte file costs about the same as a dump of a tiny one.
...
| `num_faces`, `face_data_path`     | Triangle, Quad and higher | |
| `num_polyhedra`, `polyhedron_data_path` | Tetrahedral, Hexahedral | |
