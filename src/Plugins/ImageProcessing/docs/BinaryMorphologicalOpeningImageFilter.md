# Binary Morphological Opening Image Filter
...
Open a binary image using mathematical morphology. Opening is a binary **erosion followed by a binary dilation** through the same flat structuring element (kernel) centered on each voxel. It removes foreground specks and thin protrusions smaller than the structuring element while preserving the overall shape of larger structures. A voxel is foreground only when it equals the **Foreground Value**; every other value is background. The output therefore contains only the two values `{Foreground Value, Background Value}`.
...
Opening applies binary erosion followed by dilation through the same structuring element.
...
![Binary opening example.](Images/BinaryOpening.png)
