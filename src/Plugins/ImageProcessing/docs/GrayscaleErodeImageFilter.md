...
Erode an image using grayscale morphology. Erosion takes the **minimum** of all the input values covered by the flat structuring element (kernel) centered on each voxel. Neighbors that fall outside the image bounds are skipped, which is equivalent to substituting the type maximum for out-of-image voxels (ITK's extremum-fill boundary), so they never win the minimum.

The input array must be single-component (scalar), and the output element type matches the input type. Moderate kernels on multi-plane 8-bit images with at most 96 offsets use a flat-offset minimum fold. Other data types, true-2D images, and larger kernels use the applicable moving-histogram or checked-gather path. The out-of-core path uses bounded slabs and serial bulk store transfers.
