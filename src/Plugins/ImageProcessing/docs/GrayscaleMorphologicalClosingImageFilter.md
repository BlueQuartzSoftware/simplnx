...
Grayscale morphological closing of an image.
...
Close an image using grayscale morphology. Closing is a grayscale **dilation followed by a grayscale erosion** through the same flat structuring element (kernel) centered on each voxel. It fills dark features (holes/valleys) smaller than the structuring element while preserving the overall shape and intensity of larger structures. Boundary handling near the image edge is controlled by the *Safe Border* parameter (see below).

The input array must be single-component (scalar), and the output element type matches the input type. Moderate kernels on multi-plane 8-bit images with at most 96 offsets use flat-offset maximum and minimum folds. Other data types, true-2D images, and larger kernels use the applicable moving-histogram or checked-gather path.
...
Closing applies grayscale dilation followed by erosion through the same structuring element.
...
![Grayscale closing example.](Images/GrayscaleClosing.png)
