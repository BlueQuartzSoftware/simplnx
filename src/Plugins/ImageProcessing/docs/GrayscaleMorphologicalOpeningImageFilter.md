...
Grayscale morphological opening of an image.
...
Open an image using grayscale morphology. Opening is a grayscale **erosion followed by a grayscale dilation** through the same flat structuring element (kernel) centered on each voxel. It removes bright features (peaks/specks) smaller than the structuring element while preserving the overall shape and intensity of larger structures. Boundary handling near the image edge is controlled by the *Safe Border* parameter (see below).

The input array must be single-component (scalar), and the output element type matches the input type. Moderate kernels on multi-plane 8-bit images with at most 96 offsets use flat-offset minimum and maximum folds. Other data types, true-2D images, and larger kernels use the applicable moving-histogram or checked-gather path.
...
When *Safe Border* is enabled (the default), the image is padded by the kernel radius before the erosion/dilation passes and cropped back afterward, matching the legacy ITK filter and avoiding boundary artifacts near the image edge. When disabled, the two passes run directly on the image with out-of-bounds neighbors skipped (equivalent to ITK's `SafeBorder = false`); results are identical in the image interior and differ only within one kernel radius of the border.

## Algorithm

Opening applies grayscale erosion followed by dilation through the same structuring element.

### In-Core Path

Each resident pass uses `MorphDirect`. Moderate kernels on multi-plane 8-bit images use a flat-offset fold that processes interior X rows with SIMD extrema operations and uses a checked scalar fold at the borders. Other cases use the applicable moving-histogram path.

### Out-of-Core Path

The out-of-core composite path uses Z-rings bounded by the kernel radius and serial bulk store transfers. Moderate kernels on multi-plane 8-bit images use flat-offset folds in both ring stages. Each ring stage processes interior X rows with SIMD extrema operations and uses a checked scalar fold at the borders. Other cases use the applicable moving-histogram path. The rings avoid a full intermediate array. If the ring reservation is unavailable, bounded halo slabs use the applicable fallback route. The 3D slab normally targets about 16 MiB, but it keeps at least one output plane and its complete Z halo. A wide plane can exceed the target. A Z radius at least as large as the image depth can require a full-depth input slab.

![Grayscale opening example.](Images/GrayscaleOpening.png)
