# White Top Hat Image Filter

Grayscale white top-hat transform of an image.
...
Compute the white top-hat transform of an image using grayscale morphology. The white top-hat is the **input minus its grayscale morphological opening** (an opening is an erosion followed by a dilation through the same flat structuring element). It isolates the bright features (peaks) smaller than the structuring element — those the opening removed. Boundary handling of the internal opening near the image edge is controlled by the *Safe Border* parameter (see below). Because the opening never exceeds the input, the difference is always non-negative.
...
When *Safe Border* is enabled (the default), the internal opening pads the image by the kernel radius before its erosion/dilation passes and crops back afterward, matching the legacy ITK filter and avoiding boundary artifacts near the image edge. When disabled, the opening runs directly on the image with out-of-bounds neighbors skipped (equivalent to ITK's `SafeBorder = false`); results are identical in the image interior and differ only within one kernel radius of the border.

## Algorithm

The filter performs grayscale opening and subtracts that result from the original image.

### In-Core Path

The resident erosion and dilation passes use `MorphDirect`. Moderate kernels on multi-plane 8-bit images use flat-offset folds that process interior X rows with SIMD extrema operations and use a checked scalar fold at the borders. Other cases use the applicable moving-histogram path.

### Out-of-Core Path

The out-of-core composite path uses Z-rings bounded by the kernel radius and serial bulk store transfers. Moderate kernels on multi-plane 8-bit images use flat-offset folds in both ring stages. Each ring stage processes interior X rows with SIMD extrema operations and uses a checked scalar fold at the borders. Other cases use the applicable moving-histogram path. The rings avoid a full intermediate array. If the ring reservation is unavailable, bounded halo slabs use the applicable fallback route. The 3D slab normally targets about 16 MiB, but it keeps at least one output plane and its complete Z halo. A wide plane can exceed the target. A Z radius at least as large as the image depth can require a full-depth input slab. With *Safe Border* enabled, the final crop and input-minus-opening subtraction are fused into the plane write. The subtraction processes independent output rows in parallel and does not access the data store from worker threads.

![White top-hat example.](Images/WhiteTopHat.png)
