...
Binary erosion of the foreground of an image.
...
Erode the foreground of a binary image. A voxel keeps the **Foreground Value** in the output only when **all** neighbors covered by the flat structuring element (kernel) centered on it are foreground, and is set to the **Background Value** otherwise. The output therefore contains only the two values `{Foreground Value, Background Value}`.
...
The *Boundary To Foreground* parameter controls how neighbors that fall outside the image bounds are treated: when enabled (the default for erosion), an out-of-image neighbor counts as foreground, so foreground touching the image border is not eroded away by the missing neighbors; when disabled, an out-of-image neighbor counts as background.
