#include "BinaryErodeImageFilter.hpp"
...
namespace
{
// Error/warning codes for this filter (verified free by grepping the codebase for -84xx codes; -8440..-8443 do
// not collide with any existing filter, unlike the -8410 block a naive copy from the grayscale filters would use).
constexpr int32 k_NonScalarInput = -8440;
constexpr int32 k_NonFiniteBinaryValue = -8441;
constexpr int32 k_BinaryValueOutOfRange = -8442;
constexpr int32 k_BinaryValueTruncated = -8443;
} // namespace

namespace nx::core
{
std::string BinaryErodeImageFilter::name() const
{
  return FilterTraits<BinaryErodeImageFilter>::name;
}
std::string BinaryErodeImageFilter::className() const
{
  return FilterTraits<BinaryErodeImageFilter>::className;
}
Uuid BinaryErodeImageFilter::uuid() const
{
  return FilterTraits<BinaryErodeImageFilter>::uuid;
}
std::string BinaryErodeImageFilter::humanName() const
{
  return "Binary Erode Image Filter";
}
std::vector<std::string> BinaryErodeImageFilter::defaultTags() const
{
  return {className(), "ImageProcessing", "BinaryErode", "Binary", "Morphology", "MathematicalMorphology"};
}
Parameters BinaryErodeImageFilter::parameters() const
{
  Parameters params;
  ...
  params.insert(std::make_unique<Float64Parameter>(k_ForegroundValue_Key, "Foreground Value",
                                                   "The value identifying the objects to erode. Only voxels equal to this value are foreground; all other values are background. For an integer "
                                                   "input image the value must be finite and within the image type's range.",
                                                   1.0));
  params.insert(std::make_unique<BoolParameter>(k_BoundaryToForeground_Key, "Boundary To Foreground", "Whether voxels outside the image boundary are treated as foreground during the erosion.", true));
...
IFilter::VersionType BinaryErodeImageFilter::parametersVersion() const
{
  return 1;
}
IFilter::UniquePointer BinaryErodeImageFilter::clone() const
{
  return std::make_unique<BinaryErodeImageFilter>();
}
IFilter::PreflightResult BinaryErodeImageFilter::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler,
                                                               const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  ...
  {
    return {MakeErrorResult<OutputActions>(k_NonScalarInput, fmt::format("Binary Erode requires a single-component (scalar) input array, but '{}' has {} components.", selectedInputArray.toString(),
...
Result<> BinaryErodeImageFilter::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler,
                                             const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  auto imageGeomPath = filterArgs.value<DataPath>(k_InputImageGeomPath_Key);
  auto selectedInputArray = filterArgs.value<DataPath>(k_InputImageDataPath_Key);
  ...
  return ImageProcessing::ExecuteBinaryMorphologyImageFilter(dataStructure, imageGeomPath, selectedInputArray, outputArrayPath, se, ImageProcessing::MorphOp::Erode, foregroundValue, backgroundValue,
...
Result<Arguments> BinaryErodeImageFilter::FromSIMPLJson(const nlohmann::json& json)
{
  Arguments args = BinaryErodeImageFilter().getDefaultArguments();
