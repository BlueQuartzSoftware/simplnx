...
{
  const bool isControlCharacter = fillCharacter >= 0 && fillCharacter <= 31;
  const std::string_view invalidCharacters = "{}\\/:*?\"<>|";
  return !isControlCharacter && invalidCharacters.find(fillCharacter) == std::string_view::npos;
}
} // namespace
...
Parameters WriteImageFilter::parameters() const
{
  ...
  // The mask is independent of color-table creation.
  // A linkable group cannot be the child of another linkable group.
  // Thus, the mask control stays at the top level and controls the mask parameters.
                                                         ...
                                                         const ExecutionContext& executionContext) const
{
  ...
  if(fillChar.size() != 1)
  {
    return {MakeErrorResult<OutputActions>(-27010, fmt::format("The fill character must contain exactly one character, but it contains {} characters.", fillChar.size()))};
  }
  if(!IsValidFillCharacter(fillChar.front()))
  {
    return {MakeErrorResult<OutputActions>(
        -27018, fmt::format("The fill character '{}' is not valid in an output file name. Select one printable character that is not in the set '{}'.", fillChar, "{}\\/:*?\"<>|"))};
  }
  ...
  if(maxSlice > 1)
  {
    const std::string indexStr = CreateIndexString(indexOffset, static_cast<usize>(totalDigits), fillChar);
...
Result<Arguments> WriteImageFilter::FromSIMPLJson(const nlohmann::json& json)
{
  Arguments args = WriteImageFilter().getDefaultArguments();
