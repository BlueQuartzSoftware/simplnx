...
// Mirrors ReadNrrdFile.cpp ComputeCropBounds (error codes shifted to -3584x).
Result<CropBounds> ComputeCropBounds(const nx::core::mhd::MetaImageMetadata& md, const CropGeometryParameter::ValueType& opts)
{
  CropBounds b;
  ...
  {
    return MakeErrorResult<CropBounds>(-35842, fmt::format("Invalid crop bounds: start ({}, {}, {}) must be <= end ({}, {}, {}).", b.xStart, b.yStart, b.zStart, b.xEnd, b.yEnd, b.zEnd));
  }
  if(b.xEnd >= md.dimensions[0] || b.yEnd >= md.dimensions[1] || b.zEnd >= md.dimensions[2])
  {
    return MakeErrorResult<CropBounds>(
        -35843, fmt::format("Crop end voxel ({}, {}, {}) exceeds MetaImage volume extent ({}, {}, {}).", b.xEnd, b.yEnd, b.zEnd, md.dimensions[0] - 1, md.dimensions[1] - 1, md.dimensions[2] - 1));
  }
 ...
 *        elements) per MetaImageDataReader::readBytes; out-of-range z-slices and
 *        y-rows are read and discarded. In-range x-subranges are byteswapped (if
 *        needed) into a scratch buffer and bulk-written via CopyFromArray::CopyData.
 */
template <class T>
Result<> StreamCroppedVoxels(nx::core::mhd::MetaImageDataReader& reader, AbstractDataStore<T>& store, const nx::core::mhd::MetaImageMetadata& md, const CropBounds& b,
                             const std::atomic_bool& shouldCancel, const IFilter::MessageHandler& messageHandler)
{
  ...
  for(usize srcZ = 0; srcZ < srcNz; srcZ++)
  {
    ...
    // All in-range z-slices have already been written, so stop instead of
    // reading + discarding the rest (for compressed data this also avoids
    // inflating the remainder of the volume).
    if(srcZ > b.zEnd)
    {
      break;
    }
...
namespace nx::core
{
ReadMhaFile::ReadMhaFile(DataStructure& dataStructure, const IFilter::MessageHandler& mesgHandler, const std::atomic_bool& shouldCancel, const ReadMhaFileInputValues* inputValues)
...
ReadMhaFile::~ReadMhaFile() noexcept = default;

Result<> ReadMhaFile::operator()()
{
  auto metadataResult = mhd::ReadMetaImageHeader(m_InputValues->InputFilePath);
  ...
  auto readerResult = mhd::MetaImageDataReader::Create(md);
  ...
  default:
    return MakeErrorResult(-35833, fmt::format("Internal error: unexpected output DataType {}", static_cast<int>(dataArrayBase.getDataType())));
  }
}
