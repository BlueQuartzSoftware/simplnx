#include "WhiteTopHatImageFilter.hpp"
...
namespace nx::core
{
std::string WhiteTopHatImageFilter::name() const
{
  return FilterTraits<WhiteTopHatImageFilter>::name;
}
std::string WhiteTopHatImageFilter::className() const
{
  return FilterTraits<WhiteTopHatImageFilter>::className;
}
Uuid WhiteTopHatImageFilter::uuid() const
{
  return FilterTraits<WhiteTopHatImageFilter>::uuid;
}
std::string WhiteTopHatImageFilter::humanName() const
{
  return "White Top Hat Image Filter";
}
std::vector<std::string> WhiteTopHatImageFilter::defaultTags() const
{
  return {className(), "ImageProcessing", "WhiteTopHat", "Morphology", "MathematicalMorphology"};
}
Parameters WhiteTopHatImageFilter::parameters() const
{
  Parameters params;
...
IFilter::VersionType WhiteTopHatImageFilter::parametersVersion() const
{
  return 1;
}
IFilter::UniquePointer WhiteTopHatImageFilter::clone() const
{
  return std::make_unique<WhiteTopHatImageFilter>();
}
IFilter::PreflightResult WhiteTopHatImageFilter::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler,
                                                               const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  ...
  if(inputArray.getNumberOfComponents() != 1)
  {
    return {MakeErrorResult<OutputActions>(
        -8410, fmt::format("White Top Hat requires a single-component (scalar) input array, but '{}' has {} components.", selectedInputArray.toString(), inputArray.getNumberOfComponents()))};
  }
...
Result<> WhiteTopHatImageFilter::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler,
                                             const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  auto imageGeomPath = filterArgs.value<DataPath>(k_InputImageGeomPath_Key);
  auto selectedInputArray = filterArgs.value<DataPath>(k_InputImageDataPath_Key);
  ...
  return ImageProcessing::ExecuteMorphologyCompositeImageFilter(dataStructure, imageGeomPath, selectedInputArray, outputArrayPath, se, ImageProcessing::MorphCompositeOp::WhiteTopHat, safeBorder,
...
Result<Arguments> WhiteTopHatImageFilter::FromSIMPLJson(const nlohmann::json& json)
{
  Arguments args = WhiteTopHatImageFilter().getDefaultArguments();
