...
// Kernel + Foreground + Safe Border param setter for Binary Morphological Closing. Closing has no user
...
//     (closing == erode(dilate(in))). The test drives Safe Border == FALSE so the non-padded oracle applies; the
//     ITK real-image golden below covers the padded Safe Border == true convention. It validates Annulus (the
//     filter rasterizes the real thickness-1 shell) and the (Box, 3D r{2,1,0}) cell, which has a zero-radius
//     axis on a 3D image.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// (2) Binary-input contract: the façade emits a strictly {fg, internal-bg} image and REJECTS a non-binary input
...
// -----------------------------------------------------------------------------
// (3) Both-algorithm-paths computed-expected (Safe Border == false). The binary composite routes through
...
//     SIMPLNX_TEST_ALGORITHM_PATH). The test compares the output with the independent composite fg-count oracle
//     computed over the actual input. Real-disk out-of-core coverage is retired to the generic SimplnxOoc store
//     tests.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// (4) Preflight guard: the Float64 Foreground Value is cast to the (integer) input element type; a non-finite or
...
// -----------------------------------------------------------------------------
// (5) ITK-sourced real-image md5 golden -- duplicates ITKBinaryMorphologicalClosingImageTest.cpp
...
//     WhiteDots background). The test compares our output md5 with the committed hash.
//
