 ...
 *
 * A strictly binary input is REQUIRED: this plugin's binary morphology façade accepts a strictly
 * {foreground, background} image, and it emits one. The façade's binary-input safeguard REJECTS a non-binary
 * (multi-label) input at execution. Each filter's "Rejects non-binary input at execute" test covers that path.
 */
template <class T>
 ...
 *        and pins Safe Border to @p safeBorder. Pass @c false to validate the skip-OOB composition oracle.
 */
template <class NewFilterT>
 ...
 *        Task 5's binary composites reuse it (same kernel + fg/bg/boundary shape).
 */
template <class NewFilterT>
...
namespace detail
{
// Drives @p filter with a shared Arguments over the standard geom/input/output keys plus the injected kernel
// parameters. The function then requires that preflight and execute succeed. It reads the key constants from
// NewFilterT.
template <class NewFilterT>
...
/**
 * @brief Computed-expected check for ANY kernel shape, including Annulus and a zero-radius axis. Runs
 *        @p NewFilterT with @p kernelType at @p radius and asserts the output equals an INDEPENDENT oracle:
 *        for each voxel,
 ...
 *        models the NON-padded composition (SafeBorder == false for Closing; Opening never pads), so a
 *        Closing caller MUST drive the filter with Safe Border == false. The SafeBorder cross-storage OOC
 *        check validates the padded SafeBorder == true convention.
 *
 * @pre @p se has a non-empty offset list (the empty-SE passthrough is a distinct engine path).
 */
template <class T>
 ...
 *        (Opening never pads). The oracle covers every kernel, including Annulus and a zero-radius axis.
 *        Storage is forced in-core (small image).
 *
 ...
 *        forced in-core (small correctness image). The oracle covers every kernel, including Annulus and a
 *        zero-radius axis.
 *
 * The oracle models the skip-OOB (extremum-fill at the true image edge) composition, i.e. SafeBorder == false.
 * The caller MUST therefore drive the filter with Safe Border set false (pass @ref CompositeKernelParamSetter
 * with @c safeBorder=false as @p setKernelParams).
 *
...
//------------------------------------------------------------------------------
/**
 * @brief Real-image md5 golden for a SameAsInput grayscale/binary morphology filter (plan Sec.3/4/6).
 *
 * The helper reads @p inputFile (uint8) through our reader, then runs @p FilterT with the case's kernel parameters.
 * @p setKernelParams supplies those parameters -- @ref DefaultKernelParamSetter for the grayscale primitives,
 * @ref CompositeKernelParamSetter(true) for the grayscale composites, or a binary setter that also pins
 * Foreground/Safe Border. The helper then applies the DURABLE md5 golden: our output's md5 must equal
 * @p committedMd5 (md5-validity-first, plan Sec.4). The case is pinned ForceInCore.
 */
...
inline void RunMorphologyMd5ItkGolden(const std::string& inputFile, const std::string& committedMd5, uint64 kernelType, const std::vector<uint32>& radius, const KernelParamSetter& setKernelParams)
{
  ...
  // --- run OUR filter with the case's kernel parameters ---
  Arguments args;
