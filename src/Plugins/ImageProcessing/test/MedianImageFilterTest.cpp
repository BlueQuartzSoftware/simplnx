...
// then apply the plan's md5-validity-first procedure (Sec.4): our ITK-free output's md5 must EQUAL ITK's committed
// hash. If it matches as-committed the durable golden is confirmed byte-for-byte (no re-freeze needed).
// -----------------------------------------------------------------------------
void RunMedianItkGolden(std::optional<std::vector<uint32>> radius, const std::string& committedMd5)
{
  ...
  // --- DURABLE golden: md5-validity-first (plan Sec.4): our output's md5 must equal ITK's committed hash. ---
...
// ITKMedianImageTest.cpp(defaults): the output md5 must equal the committed hash.
// -----------------------------------------------------------------------------
