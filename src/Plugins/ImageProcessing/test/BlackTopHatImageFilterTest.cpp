...
//     (black top-hat == erode(dilate(in)) - in). The test drives Safe Border == FALSE so the skip-OOB oracle
...
//     (selected by SIMPLNX_TEST_ALGORITHM_PATH). The test asserts the closed form of a Box black top-hat of a
...
// (uint8, 200x150x1), Safe Border default (true). The test compares our output md5 with the committed ITK hash. Black
