...
// Sets the geometry, input, and output keys plus MinimumObjectSize and SortByObjectSize on the Arguments. The helper
...
// -----------------------------------------------------------------------------
// (2) Execute-time overflow guard: when the number of surviving components (>= MinimumObjectSize) exceeds the
...
// -----------------------------------------------------------------------------
// (3) A request cancelled before execution must not replace or partially overwrite the caller-provided output.
...
// -----------------------------------------------------------------------------
// (4) FromSIMPLJson: MinimumObjectSize (uint64), SortByObjectSize (bool), and the geometry/array/name DataPaths.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// ITK-sourced real-image golden -- duplicates ITKRelabelComponentImageTest.cpp on our ITK-free filter. The output is
// SameAsInput (uint8 for these PNG label inputs). The durable golden is an md5 pin. The helper pins ForceInCore.
