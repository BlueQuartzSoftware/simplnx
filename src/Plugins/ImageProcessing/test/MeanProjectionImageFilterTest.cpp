...
// -----------------------------------------------------------------------------
// (2) Independent closed-form oracle (small, in-memory) for the Mean reducer. For a ramp
//     value(x,y,z) = z*D^2 + y*D + x, the mean along the projected axis (an arithmetic sequence of D terms) is
//     the term at the mid-value of that axis (see expectedForSlot below). The case asserts that term directly
//     across both output modes. Real-disk
...
// tol 1e-4. Mean is a Float64 reducer (AlwaysFloat64), only ALGEBRAICALLY equal to ITK's two-pass, hence the
// tolerant baseline comparator rather than md5/bit-exact.
