...
TEST_CASE("ImageProcessing::ReadBinaryCTNorthStarFilter: Valid filter execution")
{
...
TEST_CASE("ImageProcessing::ReadBinaryCTNorthStarFilter: Valid relative header path")
{
  UnitTest::LoadPlugins();

  const fs::path relativeFixtureDir = k_TestDirPath / "relative_northstar";
  fs::create_directories(relativeFixtureDir);
  WriteNsiHeaderFile(relativeFixtureDir / k_NsiHeaderName, k_NsiDataFileName1, k_NsiDataFileName2);
  WriteNsiBinaryDataFiles<xDim, yDim, file1ZCount, file2ZCount>(relativeFixtureDir / k_NsiDataFileName1, relativeFixtureDir / k_NsiDataFileName2);
  ...
  fs::current_path(k_TestDirPath);
  const fs::path relativeHeaderPath = fs::path("relative_northstar") / k_NsiHeaderName;

  ReadBinaryCTNorthstarFilter filter;
  DataStructure dataStructure;
  Arguments args;
  args.insertOrAssign(ReadBinaryCTNorthstarFilter::k_InputHeaderFile_Key, std::make_any<FileSystemPathParameter::ValueType>(relativeHeaderPath));
  ...
  const Float32Array& densityResult = dataStructure.getDataRefAs<Float32Array>(densityArrayPath);
  REQUIRE(densityResult.getSize() == xDim * yDim * zDim);
  for(usize index = 0; index < densityResult.getSize(); index++)
  {
    REQUIRE(densityResult[index] == index);
  }

  UnitTest::CheckArraysInheritTupleDims(dataStructure);
}

TEST_CASE("ImageProcessing::ReadBinaryCTNorthStarFilter: Valid filter execution with subvolume")
{
  UnitTest::LoadPlugins();
...
TEST_CASE("ImageProcessing::ReadBinaryCTNorthStarFilter: Invalid filter execution - Data file size too small")
{
  UnitTest::LoadPlugins();
...
TEST_CASE("ImageProcessing::ReadBinaryCTNorthStarFilter: Invalid filter execution - Data file does not exist")
{
  UnitTest::LoadPlugins();
...
TEST_CASE("ImageProcessing::ReadBinaryCTNorthStarFilter: Invalid filter execution - Incorrect Start/End Coords")
{
  UnitTest::LoadPlugins();
...
TEST_CASE("ImageProcessing::ReadBinaryCTNorthstarFilter: SIMPL Backwards Compatibility", "[ImageProcessing][ReadBinaryCTNorthstarFilter][BackwardsCompatibility]")
{
