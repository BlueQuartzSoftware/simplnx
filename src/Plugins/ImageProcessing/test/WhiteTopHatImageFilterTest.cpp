...
//     (white top-hat == in - dilate(erode(in))). The test drives Safe Border == FALSE so the skip-OOB oracle
//     applies; the ITK real-image golden below covers the Safe Border == true (padded) convention. This validates:
...
//     (selected by SIMPLNX_TEST_ALGORITHM_PATH). The test asserts the closed form of a Box white top-hat of a
//     strictly-increasing ramp directly:
...
// (uint8, 200x150x1), Safe Border default (true). The test compares our output md5 with the committed ITK hash. White
