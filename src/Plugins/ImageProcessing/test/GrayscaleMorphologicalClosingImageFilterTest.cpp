...
// -----------------------------------------------------------------------------
// (1) Computed-expected: an independent oracle composed from the skip-OOB SE-gather folds
//     (closing == erode(dilate(in))). The test drives Safe Border == FALSE so the skip-OOB oracle applies; the
//     ITK real-image golden below covers the Safe Border == true (padded) convention. This validates:
//     - a standard non-degenerate kernel under Safe Border == false (Ball + Box), and
//     - Annulus: the filter rasterizes the real thickness-1 shell.
//     - Box r{2,1,0}: a zero-radius axis on a 3D image. The oracle documents the correct result.
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// (2) Both-algorithm-paths computed-expected on a ramp (value == flat index) with a Box structuring element. The
...
//     (selected by SIMPLNX_TEST_ALGORITHM_PATH). The test asserts the closed form of a Box closing (dilate then
//     erode) of a strictly-increasing ramp directly. The dilation of a monotone ramp is again a monotone field whose value at each voxel is
...
// on STAPLE1.png (uint8, 200x150x1), Safe Border default (true). The test compares our output md5 with the
// committed ITK hash. This is a regression for the composite Safe Border 2D size-1-axis pad defect (fixed by
