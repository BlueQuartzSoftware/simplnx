...
//     (opening == dilate(erode(in))). The test drives Safe Border == FALSE so the skip-OOB oracle applies; the
//     ITK real-image goldens below cover the Safe Border == true (padded) convention. This validates:
...
// -----------------------------------------------------------------------------
// (2) Computed-expected on a ramp (value == flat index) with a Box structuring element, run on in-memory stores
...
//     scope's executeFilter witness is satisfied. The test asserts the closed form of a Box opening (erode then
//     dilate) of a strictly-increasing ramp directly. The erosion of a monotone ramp is again a monotone field whose value at each voxel is
...
// compares our output md5 with the committed ITK hash.
//
