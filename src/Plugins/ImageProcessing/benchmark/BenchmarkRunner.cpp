...
// pre-check (to catch e.g. a key mismatch without aborting the sweep), then times execute() over warmup + repeats.
...
Result<std::vector<ParsedRow>> ReadCsv(const std::string& path)
{
  ...
  const std::vector<std::string> required = {"filter", "type", "new_incore_ms", "new_ooc_ms"};
...
BenchmarkRow RunFilterBenchmark(const BenchmarkFilterSpec& spec, const SweepOptions& opts)
{
  ...
  // --- Build the IN-CORE tiled input ONCE. ---
...
std::vector<BenchmarkRow> RunSweep(const std::vector<const BenchmarkFilterSpec*>& specs, const SweepOptions& opts)
{
  ...
  for(const auto* spec : specs)
  {
    ...
    fmt::print("{:<32} type={:<8} dims={:<16} new={:>10} ooc={:>10}{}\n", row.filter, row.type.empty() ? "?" : row.type, row.inDims.empty() ? "?" : row.inDims, MsCell(row.newIncoreMs),
               MsCell(row.newOocMs), row.notes.empty() ? "" : ("  [" + row.notes + "]"));
    std::fflush(stdout);
...
Result<> WriteCsv(const std::string& path, const std::vector<BenchmarkRow>& rows)
{
  ...
  out << "filter,type,in_dims,in_voxels,new_incore_ms,new_ooc_ms,ooc_vs_incore,notes\n";
  for(const auto& r : rows)
  {
    out << CsvEscape(r.filter) << ',' << CsvEscape(r.type) << ',' << CsvEscape(r.inDims) << ',' << r.inVoxels << ',' << MsCell(r.newIncoreMs) << ',' << MsCell(r.newOocMs) << ','
        << RatioCell(r.newOocMs, r.newIncoreMs) << ',' << CsvEscape(r.notes) << '\n';
  }
  if(out.fail())
  {
