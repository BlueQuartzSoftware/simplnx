...
Parameters ResampleImageGeomFilter::parameters() const
{
  ...
  params.insert(std::make_unique<ArraySelectionParameter>(k_CellFeatureIdsArrayPath_Key, "Cell Feature Ids", "Specifies to which feature each cell belongs.",
                                                          DataPath({"Cell Data", "FeatureIds"}) ArraySelectionParameter::AllowedTypes{DataType::int32},
                                                                ...
                                                                const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  ...
  nx::core::SizeVec3 srcDimensions = srcImageGeom->getDimensions();
  nx::core::FloatVec3 srcSpacing = srcImageGeom->getSpacing();
  ...
  // not need to manually copy these arrays to the destination image geometry
  {
    ...
    preflightUpdatedValues.push_back({"Resampled Image Geometry Info", nx::core::GeometryHelpers::Description::GenerateGeometryInfo(geomDims, pSpacingValue, srcOrigin, srcImageGeom->getUnits())});
  }
