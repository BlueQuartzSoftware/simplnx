                                                    ...
                                                    const std::atomic_bool& shouldCancel, const ExecutionContext& executionContext) const
{
  ...
  {
    throttledMessenger.sendMessage(featureIdx);
  ...
  auto surfaceAreaMessenger = messageHelper.createThrottledMessenger(
      [totalFeatures](usize featureIdx) { return fmt::format("Calculating Surface Areas || {:.2f}% Complete", CalculatePercentComplete(featureIdx, totalFeatures)); });
  for(usize i = 1; i < totalFeatures; i++)
  {
    surfaceAreaMessenger.sendMessage(i);
