...
Result<> ComputeMomentInvariants2D::operator()()
{
  ...
  const auto& featureRect = m_DataStructure.getDataAs<UInt32Array>(m_InputValues->FeatureRectArrayPath)->getDataStoreRef();
  const auto numFeatures = static_cast<int32>(featureRect.getNumberOfTuples());
  auto& omega1 = m_DataStructure.getDataAs<Float32Array>(m_InputValues->Omega1ArrayPath)->getDataStoreRef();
  auto& omega2 = m_DataStructure.getDataAs<Float32Array>(m_InputValues->Omega2ArrayPath)->getDataStoreRef();
