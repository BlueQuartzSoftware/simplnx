...
namespace
{
template<bool UsingFeatures>
auto WriteOutStl(FILE* f, const IGeometry::MeshIndexType numTriangles, const std::string&& header, const IGeometry::MeshIndexArrayType& triangles, const Float32Array& vertices, const int32 featureId = -1)
{
  ...
  *attrByteCount = 0;
  ...
  for(IGeometry::MeshIndexType t = 0; t < numTriangles; ++t)
  {
...
Result<> WriteStlFile::operator()()
{
  ...
  { // Scope to cut overhead and ensure file lock is released on windows
    ...
    {
      // Faster and more memory efficent since we don't need phases
      ...
      for(const auto featureId : uniqueGrainIds)
      {
        ...
        fileList.push_back(std::make_unique<AtomicFile>(m_InputValues->OutputStlDirectory.string() + "/" + m_InputValues->OutputStlPrefix + "Feature_" + StringUtilities::number(featureId) + ".stl", false));
        ...
        FILE* f = fopen(fileList[fileIndex].tempFilePath().string().c_str(), "wb");

        if(f == nullptr)
        {
          fclose(f);
          return {MakeWarningVoidResult(-27875, fmt::format("Error Opening STL File. Unable to create temp file at path '{}' for original file '{}'", fileList[fileIndex].tempFilePath().string(), fileList[fileIndex].tempFilePath().filename().string()))};
        }

        m_MessageHandler(IFilter::Message::Type::Info, fmt::format("Writing STL for Feature Id {}", featureId));

        auto result = ::WriteOutStl<true>(f, nTriangles, {"DREAM3D Generated For Feature ID " + StringUtilities::number(featureId)}, triangles, vertices, featureId);
        // Close regardless of result; if valid Loop over all the triangles for this spin
        fclose(f);
        if(result.invalid())
        {
          return result;
    ...
    if(groupingType == GroupingType::FeaturesAndPhases)
    {
      ...
      {
        // Generate the output file
        fileList.push_back(std::make_unique<AtomicFile>(m_InputValues->OutputStlDirectory.string() + "/" + m_InputValues->OutputStlPrefix + "Ensemble_" + StringUtilities::number(value) + "_" + "Feature_" + StringUtilities::number(featureId) + ".stl", false));
        ...
        {
          fclose(f);
          return {MakeWarningVoidResult(-27876, fmt::format("Error Opening STL File. Unable to create temp file at path '{}' for original file '{}'", fileList[fileIndex].tempFilePath().string(), fileList[fileIndex].tempFilePath().filename().string()))};
        }

        m_MessageHandler(IFilter::Message::Type::Info, fmt::format("Writing STL for Feature Id {}", featureId));

        auto result = ::WriteOutStl<true>(f, nTriangles, {"DREAM3D Generated For Feature ID " + StringUtilities::number(featureId) + " Phase " + StringUtilities::number(value)}, triangles, vertices, featureId);
        // Close regardless of result; if valid Loop over all the triangles for this spin
        fclose(f);
        if(result.invalid())
        {
