...
Result<> ProcessImageGeom(ImageGeom& imageGeom, Float32AbstractDataStore& volumes, Float32AbstractDataStore& equivalentDiameters, Int32AbstractDataStore& numElements,
                          const Int32AbstractDataStore& featureIds, const bool saveElementSizes, FilterMessenger& filterMessenger, const std::atomic_bool& shouldCancel)
{
  ...
  filterMessenger.setThrottledFormatter([numVoxels](usize voxelIdx) { return fmt::format(" - Counting || {:.2f}% Complete", CalculatePercentComplete(voxelIdx, numVoxels)); });
  filterMessenger.sendInfo("Finding Voxel Counts...");
  ...
  for(usize voxelIdx = 0; voxelIdx < numVoxels; voxelIdx++)
  {
    ...
    filterMessenger.sendThrottledMessage(voxelIdx);
  ...
  {
    filterMessenger.sendInfo("Singular image detected. Proceeding with 2D calculations...");
    ...
    filterMessenger.setThrottledFormatter([numFeatures](usize featureIdx) { return fmt::format(" - Calculating || {:.2f}% Complete", CalculatePercentComplete(featureIdx, numFeatures)); });
    filterMessenger.sendInfo("Feature Level: Storing Voxel Counts and Calculating Area and ECD...");
  ...
  else
  {
    ...
    filterMessenger.sendInfo("Image Stack detected. Proceeding with 3D calculations...");

    // Calculate the volume of a single voxel
    const float64 voxelVolume = spacing[0] * spacing[1] * spacing[2];

    filterMessenger.setThrottledFormatter([numFeatures](usize featureIdx) { return fmt::format(" - Calculating || {:.2f}% Complete", CalculatePercentComplete(featureIdx, numFeatures)); });
    filterMessenger.sendInfo("Feature Level: Storing Voxel Counts and Calculating Volume and ESD...");
  ...
  if(saveElementSizes)
  {
    filterMessenger.sendInfo("Calculating Element Sizes...");
                             ...
                             const Int32AbstractDataStore& featureIds, const bool saveElementSizes, FilterMessenger& filterMessenger, const std::atomic_bool& shouldCancel)
{
  ...
  filterMessenger.sendInfo("Finding Element Sizes...");
  ...
  filterMessenger.setThrottledFormatter([numVoxels](usize voxelIdx) { return fmt::format(" - Calculating || {:.2f}% Complete", CalculatePercentComplete(voxelIdx, numVoxels)); });
  filterMessenger.sendInfo("Cell Level: Finding Voxel Counts and Summing Volumes...");
  // Count and store the number of voxels in each feature
  for(usize voxelIdx = 0; voxelIdx < numVoxels; voxelIdx++)
  {
    if(shouldCancel)
  ...
  filterMessenger.setThrottledFormatter([numFeatures](usize featureIdx) { return fmt::format(" - Calculating || {:.2f}% Complete", CalculatePercentComplete(featureIdx, numFeatures)); });
  filterMessenger.sendInfo("Feature Level: Storing Voxel Counts and Calculating ESD...");
  ...
  if(!saveElementSizes)
  {
    filterMessenger.sendInfo("Cleaning Up Element Sizes...");
...
Result<> ComputeFeatureSizes::operator()()
{
  ...
  filterMessenger.sendInfo("Validating Feature Ids and Feature Attribute Matrix...");
  ...
  {
    filterMessenger.sendInfo("Beginning Processing Features in Image Geometry...");
    auto& imageGeom = dynamic_cast<ImageGeom&>(geom);
    return ProcessImageGeom(imageGeom, volumes, equivalentDiameters, numElements, featureIds, saveElementSizes, filterMessenger, m_ShouldCancel);
  }
  if(geomType == IGeometry::Type::RectGrid)
  {
    filterMessenger.sendInfo("Beginning Processing Features in Rectilinear Grid Geometry...");
    auto& rectGridGeom = dynamic_cast<RectGridGeom&>(geom);
    return ProcessRectGridGeom(rectGridGeom, volumes, equivalentDiameters, numElements, featureIds, saveElementSizes, filterMessenger, m_ShouldCancel);
  }
