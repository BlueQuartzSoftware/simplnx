...
Result<> ComputeFeatureNeighborsScanline::operator()()
{
  ...
  // Phase 1: Loop over all points using chunk-sequential iteration
      ...
      for(usize yIdx = chunkLowerBounds[1]; yIdx <= chunkUpperBounds[1]; yIdx++)
      {
        ...
        for(usize xIdx = chunkLowerBounds[2]; xIdx <= chunkUpperBounds[2]; xIdx++)
        {
          ...
          if(feature > 0 && feature < neighborlist.size())
          {
            ...
            const int64 iy = static_cast<int64>(yIdx);
            const int64 iz = static_cast<int64>(zIdx);

            if(storeSurfaceFeatures && surfaceFeatures != nullptr)
            {
              if((ix == 0 || ix == dims[0] - 1 || iy == 0 || iy == dims[1] - 1 || iz == 0 || iz == dims[2] - 1) && dims[2] != 1)
              {
                surfaceFeatures->setValue(feature, true);
              }
              if((ix == 0 || ix == dims[0] - 1 || iy == 0 || iy == dims[1] - 1) && dims[2] == 1)
              {
                surfaceFeatures->setValue(feature, true);
              }
            }
            ...
            std::array<bool, 6> isValidFaceNeighbor = computeValidFaceNeighbors(ix, iy, iz, dims);
