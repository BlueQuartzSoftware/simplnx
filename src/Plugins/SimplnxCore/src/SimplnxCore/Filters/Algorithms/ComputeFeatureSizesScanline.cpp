...
Result<> StoreFeatureOutputs(usize numFeatures, const std::vector<uint64>& featureVoxelCounts, FeatureSizeFunctorT&& featureSizeFunctor, EquivalentDiameterFunctorT&& equivalentDiameterFunctor,
                             Int32AbstractDataStore& numElements, Float32AbstractDataStore& volumes, Float32AbstractDataStore& equivalentDiameters, ThrottledMessageHandler& throttledMessenger,
                             const std::atomic_bool& shouldCancel)
{
  ...
  for(usize featureIdx = 1; featureIdx < numFeatures; featureIdx++)
  {
    ...
    throttledMessenger.queueMessage([&] { return fmt::format(" - Calculating || {:.2f}% Complete", CalculatePercentComplete(featureIdx, numFeatures)); });
...
Result<> ProcessImageGeom(ImageGeom& imageGeom, Float32AbstractDataStore& volumes, Float32AbstractDataStore& equivalentDiameters, Int32AbstractDataStore& numElements,
                          ...
                          const IFilter::MessageHandler& msgHelper, const std::atomic_bool& shouldCancel)
{
  ThrottledMessageHandler throttledMessenger(msgHelper);
  ...
  msgHelper.sendInfoMessage("Finding Voxel Counts...");
  ...
  {
    msgHelper.sendInfoMessage("Singular image detected. Proceeding with 2D calculations...");
    ...
    msgHelper.sendInfoMessage("Feature Level: Storing Voxel Counts and Calculating Area and ECD...");
  ...
  {
    msgHelper.sendInfoMessage("Image Stack detected. Proceeding with 3D calculations...");

    const float64 voxelVolume = spacing[0] * spacing[1] * spacing[2];

    msgHelper.sendInfoMessage("Feature Level: Storing Voxel Counts and Calculating Volume and ESD...");
    Result<> outputResult = StoreFeatureOutputs(
  ...
  if(saveElementSizes)
  {
    msgHelper.sendInfoMessage("Calculating Element Sizes...");
                             ...
                             const IFilter::MessageHandler& msgHelper, const std::atomic_bool& shouldCancel)
{
  ...
  msgHelper.sendInfoMessage("Finding Element Sizes...");
  ...
  msgHelper.sendInfoMessage("Cell Level: Finding Voxel Counts and Summing Volumes...");
  ...
  msgHelper.sendInfoMessage("Feature Level: Storing Voxel Counts and Calculating ESD...");
  Result<> outputResult = StoreFeatureOutputs(
  ...
  if(!saveElementSizes)
  {
    msgHelper.sendInfoMessage("Cleaning Up Element Sizes...");
...
Result<> ComputeFeatureSizesScanline::operator()()
{
  ...
  messageHelper.sendInfoMessage("Validating Feature Ids and Feature Attribute Matrix...");
  ...
  {
    messageHelper.sendInfoMessage("Beginning Processing Features in Image Geometry...");
  ...
  {
    messageHelper.sendInfoMessage("Beginning Processing Features in Rectilinear Grid Geometry...");
