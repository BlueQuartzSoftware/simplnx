...
| Legacy comparison      | **Run** on the SmallIn100 fixture (`6_6_stats_test_v2.tar.gz`). NumNeighbors, NeighborList, and SurfaceFeatures are bit-identical on SmallIn100 (a proper 3D dataset — D2 is only observable on 1D/EmptyY/EmptyX inputs). SSA differs (D1 — legacy bug). BoundaryCells not present in archive. |
