...
| Oracle (confirmed)     | **Class 1 (Analytical) primary** — hand-derivation embedded in the Oracle section (set-union sums + ratio division on the 5-feature v2 toy dataset; 4-configuration expected-output table). **Class 4 (Invariant) companion** — sentinel `-1.0f` on empty parents; `density[0] == 0.0f`; `density[i] ≤ 1.0f`; `density[i] > 0 ∨ == -1.0f`; `CheckedFeatures` range — asserted inline. **Second-engineer review skipped** (recorded reason): 5-feature toy dataset arithmetic is high-school-level set-union + ratio; bit-identical cross-check against an independently-built legacy serves as external validation.                                                  |
...
| Exemplar archive       | `compute_grouping_densities_v2.tar.gz` (SHA512 `3aaabb63…3784ff`, wired into `test/CMakeLists.txt`). **Replaces** v1 archive `compute_grouping_densities.tar.gz` (retired from this filter's tests). The v2 archive carries inline provenance: input file + all 4 legacy/SIMPLNX outputs + comparison script + comparison report + legacy/SIMPLNX pipelines + hand-review sign-off ReadMe. **No separate `vv/provenance/` file** — provenance is materially captured in the archive's ReadMe; OneDrive duplication can be done at SBIR deliverable assembly.                                                                                                              |
| Legacy comparison      | **Empirical A/B against locally-rebuilt legacy** `FindGroupingDensity` (`/Users/mjackson/DREAM3D-Dev/DREAM3D` on the 6.5.172 branch with the `feature/770_Grouping_Density` sources pulled in). All 4 `(UseNonContiguousNeighbors, FindCheckedFeatures)` configurations produce **BIT-IDENTICAL** `GroupingDensities` and `CheckedFeatures` output between SIMPLNX and the rebuilt legacy. **0 deviations.** See `vv/deviations/ComputeGroupingDensityFilter.md` for the per-configuration result table, comparison method, build provenance, and the migration recommendation for the legacy-custom-build customers (*trust SIMPLNX — output is bit-identical*).             |
