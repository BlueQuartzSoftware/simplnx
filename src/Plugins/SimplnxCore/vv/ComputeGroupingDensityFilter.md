...
| Code paths enumerated | 7 of 8 listed paths exercised; cancellation remains outside this scope. |
| Tests today | 7 registered cases plus one hidden HDF5 boundary case. Both serial CTest selections pass 7/7. |
...
OOC source: `Algorithms/ComputeGroupingDensity.cpp` (238 lines). The new test targets the storage path documented below.
...
| HDF5 list chunk boundary | `real HDF5 neighbor-list chunk oracle` — features 2/3 cross three-tuple list chunks; exact 0.75 or 0.5 density and checked-feature claims |
...
| `real HDF5 neighbor-list chunk oracle` | new-for-V&V | Actual HDF5 input/output and independent boundary expectations; 143 assertions across the configured options. |
...
OOC recertification (2026-09-21): serial CTest passed 7/7 in each DREAM3DNX build. `test/ComputeGroupingDensityTest.cpp::real HDF5 neighbor-list chunk oracle` passed 143 assertions in the OOC binary. All four option combinations run on HDF5 numeric arrays and HDF5 NeighborLists with shape {3,1,3}. The current list factory stores each outer slice in a separate chunk. Feature 2 is the last tuple of chunk zero and feature 3 starts chunk one. Duplicate neighbor 4 counts once; optional neighbor 8 changes the denominator from 16 to 24, giving exact densities 12/16=0.75 and 12/24=0.5. Parent 2 remains -1. The legacy baseline is still the documented custom source build, not a stock 6.5.171 importer. Upstream/develop oracle assertions and tolerances remain intact. No fresh legacy binary comparison is claimed.
