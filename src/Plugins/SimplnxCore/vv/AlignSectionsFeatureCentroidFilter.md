...
|---|---|
| Algorithm Relationship | **Port of DREAM3D 6.5.171 `AlignSectionsFeatureCentroid`, with one deliberate output rewrite plus four corrections.** The centroid reduction, the top-down iteration order, the truncating shift conversion, the cumulative composition and the shift-application math are line-for-line ports. The shift *output* was rewritten from a CSV file to four Data Arrays (PR #1237). This V&V corrected four defects: the Reference Slice semantics (shared with legacy), the all-masked-slice NaN cast (shared with legacy), missing preflight guards (SIMPLNX-only) and uninitialized shift-array tuple 0 (SIMPLNX-only). |
...
| Code paths enumerated | **18 of 24 exercised** (centroid reduction, empty slice, empty reference slice, reference vs consecutive, store vs no-store, positive/negative traversal, zero-shift skip, zero fill, range warnings, cancel, seven preflight guard codes). The six uncovered paths are listed as rows in `## Code path coverage`. |
...
| Bug flags | **D1, D2, D3, D4, D7, D8, D9, D12** — every deviation whose root cause is `bug`. Shared with legacy: D1, D2. Legacy-only: D3, D4, D12. SIMPLNX-only: D7, D8, D9. (D13 is dead code plus a documentation error with no computational effect.) |
...
| D13 | Dead error-code constants `-68001..-68004`, plus a documentation paragraph describing an option this filter has never had | `bug` (dead code and a documentation error; no computational effect) | **Removed.** Fix round 1 additionally removed the `-68063` tuple-count guard, which called `validateNumberOfTuples` with a single path and therefore could never fail; the real check is now `-68075` (D9). |
