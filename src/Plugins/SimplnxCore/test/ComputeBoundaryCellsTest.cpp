...
TEST_CASE("SimplnxCore::ComputeBoundaryCellsFilter: SIMPL Backwards Compatibility", "[SimplnxCore][ComputeBoundaryCellsFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ComputeBoundaryCellsFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ComputeBoundaryCellsFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ComputeBoundaryCellsFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<bool>(ComputeBoundaryCellsFilter::k_IgnoreFeatureZero_Key) == true);
      CHECK(args.value<bool>(ComputeBoundaryCellsFilter::k_IncludeVolumeBoundary_Key) == true);
      CHECK(args.value<DataPath>(ComputeBoundaryCellsFilter::k_GeometryPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ComputeBoundaryCellsFilter::k_FeatureIdsArrayPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ComputeBoundaryCellsFilter::k_BoundaryCellsArrayName_Key) == "TestName");
    }
  }
}
