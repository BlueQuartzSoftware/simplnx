...
#include "SimplnxCore/Filters/Algorithms/SurfaceNetsScanline.hpp"
...
struct SurfaceNetsFixture
{
  ...
  static void Create(DataStructure& dataStructure, bool useConfiguredStores, bool featureIdsInMemory = false, bool selectedCellOnly = false)
  {
    ...
    {
      featureIdsStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_SmallFeatureIdsPath, k_CellShape, {1});
    }
    else
    {
    ...
    auto cellIntStore = useConfiguredStores ? DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_SmallCellIntPath, k_CellShape, {2}) :
                                              std::make_shared<DataStore<int32>>(k_CellShape, ShapeType{2}, std::optional<int32>{});
    auto cellFloatStore = useConfiguredStores && !selectedCellOnly ? DataStoreUtilities::CreateDataStore<float32>(dataStructure, k_SmallCellFloatPath, k_CellShape, {3}) :
                                                                     std::make_shared<DataStore<float32>>(k_CellShape, ShapeType{3}, std::optional<float32>{});
    auto cellBoolStore = useConfiguredStores && !selectedCellOnly ? DataStoreUtilities::CreateDataStore<bool>(dataStructure, k_SmallCellBoolPath, k_CellShape, {1}) :
                                                                    std::make_shared<DataStore<bool>>(k_CellShape, ShapeType{1}, std::optional<bool>{});
    auto featureStore = useConfiguredStores && !selectedCellOnly ? DataStoreUtilities::CreateDataStore<float32>(dataStructure, k_SmallFeatureVectorPath, k_FeatureShape, {3}) :
    ...
    auto featureIdsStoreWriteResult = featureIdsStore->copyFromBuffer(0, nonstd::span<const int32>(featureIds.data(), featureIds.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(featureIdsStoreWriteResult);
    auto cellIntStoreWriteResult = cellIntStore->copyFromBuffer(0, nonstd::span<const int32>(cellInts.data(), cellInts.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(cellIntStoreWriteResult);
    auto cellFloatStoreWriteResult = cellFloatStore->copyFromBuffer(0, nonstd::span<const float32>(cellFloats.data(), cellFloats.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(cellFloatStoreWriteResult);
    auto cellBoolStoreWriteResult = cellBoolStore->copyFromBuffer(0, nonstd::span<const bool>(cellBools.get(), featureIds.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(cellBoolStoreWriteResult);
    auto featureStoreWriteResult = featureStore->copyFromBuffer(0, nonstd::span<const float32>(featureValues.data(), featureValues.size()));
...
/**
 * @brief Checks the generated mesh and transferred values against the fixture.
 * @param dataStructure Contains one selected algorithm's output.
 */
void RequireSmallOutputValues(const DataStructure& dataStructure)
{
  const auto facePath = k_SmallOutputPath.createChildPath(k_FaceDataGroupName);
  const auto facesPath = k_SmallOutputPath.createChildPath(TriangleGeom::k_SharedFacesListName);
  const auto verticesPath = k_SmallOutputPath.createChildPath(INodeGeometry0D::k_SharedVertexListName);
  REQUIRE_NOTHROW(dataStructure.getDataRefAs<DataArray<IGeometry::MeshIndexType>>(facesPath));
  REQUIRE_NOTHROW(dataStructure.getDataRefAs<Float32Array>(verticesPath));
  REQUIRE_NOTHROW(dataStructure.getDataRefAs<Int32Array>(facePath.createChildPath(k_Face_Labels)));
  REQUIRE_NOTHROW(dataStructure.getDataRefAs<Int32Array>(facePath.createChildPath("CellInt")));
  REQUIRE_NOTHROW(dataStructure.getDataRefAs<Float32Array>(facePath.createChildPath("CellFloat")));
  REQUIRE_NOTHROW(dataStructure.getDataRefAs<BoolArray>(facePath.createChildPath("CellBool")));
  REQUIRE_NOTHROW(dataStructure.getDataRefAs<Float32Array>(facePath.createChildPath("FeatureVector")));
  const auto& faces = dataStructure.getDataRefAs<DataArray<IGeometry::MeshIndexType>>(facesPath);
  const auto& vertices = dataStructure.getDataRefAs<Float32Array>(verticesPath);
  const auto& labels = dataStructure.getDataRefAs<Int32Array>(facePath.createChildPath(k_Face_Labels));
  const auto& ints = dataStructure.getDataRefAs<Int32Array>(facePath.createChildPath("CellInt"));
  const auto& floats = dataStructure.getDataRefAs<Float32Array>(facePath.createChildPath("CellFloat"));
  const auto& bools = dataStructure.getDataRefAs<BoolArray>(facePath.createChildPath("CellBool"));
  const auto& features = dataStructure.getDataRefAs<Float32Array>(facePath.createChildPath("FeatureVector"));
  // The 4x3x2 box has 52 exterior voxel faces. Its labels add 12
  // internal y-faces and four internal x-faces. Each face gives two triangles.
  constexpr usize k_TriangleCount = 2 * (52 + 12 + 4);
  REQUIRE(faces.getNumberOfTuples() == k_TriangleCount);
  REQUIRE(faces.getNumberOfComponents() == 3);
  REQUIRE(vertices.getNumberOfComponents() == 3);
  REQUIRE(vertices.getNumberOfTuples() > 0);
  REQUIRE(labels.getSize() == 2 * k_TriangleCount);
  REQUIRE(ints.getSize() == 4 * k_TriangleCount);
  REQUIRE(floats.getSize() == 6 * k_TriangleCount);
  REQUIRE(bools.getSize() == 2 * k_TriangleCount);
  REQUIRE(features.getSize() == 6 * k_TriangleCount);
  for(usize valueIdx = 0; valueIdx < vertices.getSize(); ++valueIdx)
  {
    CHECK(std::isfinite(vertices[valueIdx]));
  }
  for(usize faceIdx = 0; faceIdx < k_TriangleCount; ++faceIdx)
  {
    CAPTURE(faceIdx);
    for(usize compIdx = 0; compIdx < 3; ++compIdx)
    {
      REQUIRE(faces[3 * faceIdx + compIdx] < vertices.getNumberOfTuples());
      CHECK(faces[3 * faceIdx + compIdx] != faces[3 * faceIdx + (compIdx + 1) % 3]);
    }
    std::array<int32, 2> sourceLabels = {};
    for(usize sideIdx = 0; sideIdx < 2; ++sideIdx)
    {
      const usize side = 2 * faceIdx + sideIdx;
      // CellFloat's first component distinguishes exterior zeroes from
      // input cell zero, whose first component is 0.25.
      if(floats[3 * side] == 0.0F)
      {
        sourceLabels[sideIdx] = -1;
        CHECK(ints[2 * side] == 0);
        CHECK(ints[2 * side + 1] == 0);
        CHECK_FALSE(bools[side]);
        for(usize compIdx = 0; compIdx < 3; ++compIdx)
        {
          CHECK(floats[3 * side + compIdx] == 0.0F);
          CHECK(features[3 * side + compIdx] == 0.0F);
        }
        continue;
      }
      const int32 sourceIdx = ints[2 * side];
      REQUIRE(sourceIdx >= 0);
      REQUIRE(sourceIdx < 24);
      const int32 label = sourceIdx % 4 == 0 ? 1 : (sourceIdx / 4) % 3 + 1;
      sourceLabels[sideIdx] = label;
      CHECK(ints[2 * side + 1] == -sourceIdx);
      CHECK(floats[3 * side] == static_cast<float32>(sourceIdx) + 0.25F);
      CHECK(floats[3 * side + 1] == -static_cast<float32>(sourceIdx) - 0.5F);
      CHECK(floats[3 * side + 2] == static_cast<float32>(sourceIdx % 5) * 1.25F);
      CHECK(bools[side] == (sourceIdx % 2 == 0));
      for(usize compIdx = 0; compIdx < 3; ++compIdx)
      {
        CHECK(features[3 * side + compIdx] == static_cast<float32>(3 * label + compIdx) + 0.125F);
      }
    }
    // Face labels are sorted; tuple transfer preserves source-side order.
    CHECK(labels[2 * faceIdx] == std::min(sourceLabels[0], sourceLabels[1]));
    CHECK(labels[2 * faceIdx + 1] == std::max(sourceLabels[0], sourceLabels[1]));
    CHECK(labels[2 * faceIdx] < labels[2 * faceIdx + 1]);
  }
}
...
void CreateOutputs(DataStructure& dataStructure, const Arguments& args)
{
  ...
  const auto values = CreateInputValues(args);
  UnitTest::RequireAutomaticCreateArrayActions(preflight.outputActions, 2 + values.CreatedDataArrayPaths.size());
  std::vector<DataPath> expectedArrayPaths = {values.NodeTypesDataPath, values.FaceLabelsDataPath};
  expectedArrayPaths.insert(expectedArrayPaths.end(), values.CreatedDataArrayPaths.begin(), values.CreatedDataArrayPaths.end());
...
{
  int32 value = 0;
  auto storeReadResult = store.copyIntoBuffer(offset, nonstd::span<int32>(&value, 1));
  SIMPLNX_RESULT_REQUIRE_VALID(storeReadResult);
...
TEST_CASE("SimplnxCore::SurfaceNetsFilter: generated direct and scanline parity", "[SimplnxCore][SurfaceNetsFilter]")
{
  ...
  for(const bool smoothing : {false, true})
  {
    ...
    for(const auto scenario : scenarios)
    {
      ...
      SurfaceNetsFilter filter;
      auto result = scope.executeFilter(filter, dataStructure, SurfaceNetsFixture::ArgumentsFor(smoothing));
      SIMPLNX_RESULT_REQUIRE_VALID(result.result);
      RequireSmallOutputValues(dataStructure);
      UnitTest::CheckArraysInheritTupleDims(dataStructure);
      if(hasExpectedData)
      {
        CompareSmallOutputs(expectedData, dataStructure);
      }
      else
      {
...
TEST_CASE("SimplnxCore::SurfaceNets: scanline bulk failures propagate", "[SimplnxCore][SurfaceNetsFilter]")
{
  ...
  SECTION("FeatureIds classification read")
  {
    ...
    auto result = SurfaceNetsScanline(dataStructure, {}, shouldCancel, &values)();
...
TEST_CASE("SimplnxCore::TupleTransfer: large-component direct fallback", "[SimplnxCore][SurfaceNetsFilter][OOC]")
{
  ...
  auto sourceWriteResult = source->getDataStoreRef().copyFromBuffer(0, nonstd::span<const int32>(sourceValues.get(), 3 * kComponents));
  SIMPLNX_RESULT_REQUIRE_VALID(sourceWriteResult);
  ...
  auto surfaceNetsTransferResult = transfer.surfaceNetsTransferBatch(nonstd::span<const SurfaceNetsTransferData>(records.data(), records.size()));
  SIMPLNX_RESULT_REQUIRE_VALID(surfaceNetsTransferResult);
