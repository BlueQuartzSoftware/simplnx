...
void BuildBenchmarkDataStructure(DataStructure& dataStructure)
{
  ...
  auto inputStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_InputArrayPath, cellTupleShape, {1});
  ...
  {
    auto inputStoreWriteResult = inputStore->copyFromBuffer(z * k_BenchmarkSliceTuples, nonstd::span<const int32>(sliceBuffer.get(), k_BenchmarkSliceTuples));
  ...
  auto boundsStore = DataStoreUtilities::CreateDataStore<float32>(dataStructure, k_UnifiedBoundsPath, {k_BenchmarkNumBounds}, {k_BenchmarkBoundsComponents});
  ...
  auto boundsStoreWriteResult = boundsStore->copyFromBuffer(0, nonstd::span<const float32>(bounds.data(), bounds.size()));
  SIMPLNX_RESULT_REQUIRE_VALID(boundsStoreWriteResult);
}
} // namespace
