...
void BuildLargeTestData(DataStructure& dataStructure, bool includeBlockedCells = false)
{
  ...
  auto featureIdsStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_LargeFeatureIdsPath, cellTupleShape, {1});
  ...
  for(usize z = 0; z < k_LargeDimZ; z++)
  {
    ...
    auto featureIdsRefWriteResult = featureIdsRef.copyFromBuffer(z * k_LargeSliceSize, nonstd::span<const int32>(sliceBuffer.data(), sliceBuffer.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(featureIdsRefWriteResult);
  }
}
...
uint64 HashInt32Store(const AbstractDataStore<int32>& dataStore)
{
  ...
  for(usize offset = 0; offset < dataStore.getSize(); offset += buffer.size())
  {
    ...
    auto dataStoreReadResult = dataStore.copyIntoBuffer(offset, nonstd::span<int32>(buffer.data(), count));
    SIMPLNX_RESULT_REQUIRE_VALID(dataStoreReadResult);
...
uint64 HashDataStoreBytes(const AbstractDataStore<T>& dataStore)
{
  ...
  {
    const usize count = std::min(buffer.size(), dataStore.getSize() - offset);
    auto dataStoreReadResult = dataStore.copyIntoBuffer(offset, nonstd::span<T>(buffer.data(), count));
    SIMPLNX_RESULT_REQUIRE_VALID(dataStoreReadResult);
...
void BuildComparisonTestData(DataStructure& dataStructure, bool includeBlockedCells)
{
  ...
  for(usize z = 0; z < k_DimZ; z++)
  {
    ...
    auto featureIdsRefWriteResult = featureIdsRef.copyFromBuffer(z * k_SliceSize, nonstd::span<const int32>(sliceBuffer.data(), sliceBuffer.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(featureIdsRefWriteResult);
  }
}
  ...
  DYNAMIC_SECTION("blocked cells: " << includeBlockedCells)
  {
    ...
    for(const auto scenario : scenarios)
    {
      ...
      for(usize outputIndex = 0; outputIndex < outputNames.size(); outputIndex++)
      {
        ...
        auto outputArrayReadResult = outputArray.getDataStoreRef().copyIntoBuffer(0, nonstd::span<float32>(outputValues.data(), outputValues.size()));
        SIMPLNX_RESULT_REQUIRE_VALID(outputArrayReadResult);
...
TEST_CASE("SimplnxCore::ComputeEuclideanDistMap: 200x200x200 same-storage algorithm matrix", "[SimplnxCore][ComputeEuclideanDistMap][.Performance]")
{
  ...
  {
    std::array<uint64, 3> expectedHashes = {};
    bool hasExpectedHashes = false;
    const auto scenarios = UnitTest::SelectAlgorithmTestScenariosForInMemoryStores();
    for(const auto scenario : scenarios)
    {
      CAPTURE(scenario);
      UnitTest::AlgorithmTestScope scope(scenario);
      DataStructure dataStructure;
      BuildLargeTestData(dataStructure, includeBlockedCells);
      REQUIRE_NOTHROW(dataStructure.getDataRefAs<Int32Array>(k_LargeFeatureIdsPath));
      scope.requireExpectedStore(dataStructure.getDataRefAs<Int32Array>(k_LargeFeatureIdsPath));
      scope.execute(
          [&dataStructure, shouldCalculateManhattanDistance, shouldCalculateAllMaps] { return ExecuteTimedDistanceMap(dataStructure, shouldCalculateManhattanDistance, shouldCalculateAllMaps); });
      const auto hashes = HashDistanceMaps(dataStructure, calcManhattanDist, calculateAllMaps);
      // The first block interface is at coordinates 24/25, far from the
      // optional x=100 barrier. These sites are seeds or one x-step from
      // the only nearest seed, so anisotropic spacing has no tie ambiguity.
      const std::array<std::string, 3> outputNames = {k_GBDistancesArrayName, k_TJDistancesArrayName, k_QPDistancesArrayName};
      const std::array<std::array<usize, 3>, 3> nearSeed = {{{23, 12, 12}, {23, 24, 12}, {23, 24, 24}}};
      for(usize mapIdx = 0; mapIdx < (calculateAllMaps ? 3 : 1); ++mapIdx)
      {
        const DataPath outputPath = k_LargeCellDataPath.createChildPath(outputNames[mapIdx]);
        const auto requireDistance = [&](usize x, usize y, usize z, float64 expected) {
          const usize index = z * k_LargeSliceSize + y * k_LargeDimX + x;
          if(calcManhattanDist)
          {
            REQUIRE_NOTHROW(dataStructure.getDataRefAs<Int32Array>(outputPath));
            CHECK(dataStructure.getDataRefAs<Int32Array>(outputPath)[index] == static_cast<int32>(expected));
          }
          else
          {
            REQUIRE_NOTHROW(dataStructure.getDataRefAs<Float32Array>(outputPath));
            CHECK(dataStructure.getDataRefAs<Float32Array>(outputPath)[index] == Approx(expected));
          }
        };
        CAPTURE(mapIdx);
        requireDistance(24, 24, 24, 0.0);
        requireDistance(nearSeed[mapIdx][0], nearSeed[mapIdx][1], nearSeed[mapIdx][2], calcManhattanDist ? 1.0 : 0.5);
        if(includeBlockedCells)
        {
          requireDistance(100, 10, 10, -1.0);
        }
      }
      if(hasExpectedHashes)
      {
        CHECK(hashes == expectedHashes);
      }
      else
      {
        expectedHashes = hashes;
        hasExpectedHashes = true;
      }
      UnitTest::CheckArraysInheritTupleDims(dataStructure);
    }
  }
