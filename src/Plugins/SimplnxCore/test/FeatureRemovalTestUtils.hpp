...
inline void BuildInput(DataStructure& dataStructure)
{
  ...
  for(usize z = 0; z < k_Dim; z++)
  {
    ...
    auto featureIdsWriteResult = featureIds.getDataStoreRef().copyFromBuffer(z * k_SliceTuples, nonstd::span<const int32>(featureIdSlice.data(), featureIdSlice.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(featureIdsWriteResult);
    auto cellValuesWriteResult = cellValues.getDataStoreRef().copyFromBuffer(z * k_SliceTuples * k_ValueComponents, nonstd::span<const float32>(valueSlice.data(), valueSlice.size()));
    SIMPLNX_RESULT_REQUIRE_VALID(cellValuesWriteResult);
  }
  ...
  auto numCellsWriteResult = numCells.getDataStoreRef().copyFromBuffer(0, nonstd::span<const int32>(numCellValues.data(), numCellValues.size()));
  SIMPLNX_RESULT_REQUIRE_VALID(numCellsWriteResult);
  auto numNeighborsWriteResult = numNeighbors.getDataStoreRef().copyFromBuffer(0, nonstd::span<const int32>(neighborValues.data(), neighborValues.size()));
  SIMPLNX_RESULT_REQUIRE_VALID(numNeighborsWriteResult);
  auto featureMarkerWriteResult = featureMarker.getDataStoreRef().copyFromBuffer(0, nonstd::span<const int32>(markerValues.data(), markerValues.size()));
  SIMPLNX_RESULT_REQUIRE_VALID(featureMarkerWriteResult);
}
...
uint64 HashStore(const AbstractDataStore<T>& store)
{
  ...
  for(usize offset = 0; offset < store.getSize(); offset += buffer.size())
  {
    ...
    auto storeReadResult = store.copyIntoBuffer(offset, nonstd::span<T>(buffer.data(), count));
