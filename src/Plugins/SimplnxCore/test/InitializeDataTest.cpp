...
DataStructure CreateBulkInitializationData(bool configuredStores)
{
  ...
  if(configuredStores)
  {
    intStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_BulkIntPath, {k_BulkTupleCount}, {k_BulkComponentCount});
    boolStore = DataStoreUtilities::CreateDataStore<bool>(dataStructure, k_BulkBoolPath, {k_BulkTupleCount}, {k_BulkComponentCount});
  }
  else
  {
...
std::vector<T> ReadBulkValues(const DataStructure& dataStructure, const DataPath& path)
{
  ...
  auto arrayReadResult = array.getDataStoreRef().copyIntoBuffer(0, nonstd::span<T>(values.get(), array.getSize()));
