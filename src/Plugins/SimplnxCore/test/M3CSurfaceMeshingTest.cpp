...
DataStructure CreateM3CParityData(bool useConfiguredCellStores)
{
  ...
  {
    featureIdsStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_ParityFeatureIdsPath, tupleShape, {1});
    cellVectorsStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_ParityCellVectorsPath, tupleShape, {2});
  }
  else
  {
  ...
  auto featureIdsWriteResult = featureIds->getDataStoreRef().copyFromBuffer(0, featureIdValues);
  SIMPLNX_RESULT_REQUIRE_VALID(featureIdsWriteResult);
  auto cellVectorsWriteResult = cellVectors->getDataStoreRef().copyFromBuffer(0, cellVectorValues);
  SIMPLNX_RESULT_REQUIRE_VALID(cellVectorsWriteResult);
  auto featureValuesWriteResult = featureValues->getDataStoreRef().copyFromBuffer(0, featureValueBuffer);
  SIMPLNX_RESULT_REQUIRE_VALID(featureValuesWriteResult);
