...
void BuildBenchmarkInput(DataStructure& dataStructure)
{
  ...
  auto featureIdsStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_BenchmarkFeatureIdsPath, cellTupleShape, {1});
  ...
  auto boundaryCellsStore = DataStoreUtilities::CreateDataStore<int8>(dataStructure, k_BenchmarkBoundaryCellsPath, cellTupleShape, {1});
  ...
  for(usize z = 0; z < k_BenchmarkDim; z++)
  {
    ...
    auto featureIdsStoreWriteResult = featureIdsStore->copyFromBuffer(offset, nonstd::span<const int32>(featureIdsBuffer.get(), k_BenchmarkSliceTuples));
    SIMPLNX_RESULT_REQUIRE_VALID(featureIdsStoreWriteResult);
    auto boundaryCellsStoreWriteResult = boundaryCellsStore->copyFromBuffer(offset, nonstd::span<const int8>(boundaryCellsBuffer.get(), k_BenchmarkSliceTuples));
    SIMPLNX_RESULT_REQUIRE_VALID(boundaryCellsStoreWriteResult);
  }
