...
void BuildTestData(DataStructure& dataStructure, usize dimX, usize dimY, usize dimZ)
{
  ...
  auto confDataStore = DataStoreUtilities::CreateDataStore<float32>(dataStructure, k_ConfidencePath, cellTupleShape, {1});
  ...
  auto phasesDataStore = DataStoreUtilities::CreateDataStore<int32>(dataStructure, k_CellDataPath.createChildPath("Phases"), cellTupleShape, {1});
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    ...
    auto confStoreWriteResult = confStore.copyFromBuffer(zOffset, nonstd::span<const float32>(confBuf.data(), sliceSize));
    SIMPLNX_RESULT_REQUIRE_VALID(confStoreWriteResult);
...
usize CountVoxelsBelowThreshold(const DataStructure& dataStructure, float32 threshold, usize dimX, usize dimY, usize dimZ)
{
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    auto confReadResult = conf.copyIntoBuffer(z * sliceSize, nonstd::span<float32>(buf.data(), sliceSize));
    SIMPLNX_RESULT_REQUIRE_VALID(confReadResult);
