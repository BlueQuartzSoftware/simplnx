...
Result<> ExecuteProcess::operator()()
{
  ...
  if(m_InputValues->Blocking)
  {
    ...
    reproc::stop_actions stop = {
        {reproc::stop::noop, reproc::milliseconds(0)}, {reproc::stop::terminate, reproc::milliseconds(m_InputValues->Timeout)}, {reproc::stop::kill, reproc::milliseconds(m_InputValues->Timeout)}};
    options.stop = stop;
    ...
    if(ec)
    {
      return MakeErrorResult<>(-56411, fmt::format("An error occured while executing process '{}'\n{} : {}", m_InputValues->Arguments, ec.value(), ec.message()));
    }
  ...
  {
    reproc::options options;
    reproc::stop_actions stop = {{reproc::stop::terminate, reproc::milliseconds(2000)}, {reproc::stop::kill, reproc::milliseconds(10000)}};
    options.stop = stop;
    std::error_code ec = process.start(args, options);
    ...
    while(drain_async.wait_for(std::chrono::seconds(2)) != std::future_status::ready)
    {
