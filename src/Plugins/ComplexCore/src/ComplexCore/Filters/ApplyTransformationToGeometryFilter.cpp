...
Parameters ApplyTransformationToGeometryFilter::parameters() const
{
  ...
  params.insert(std::make_unique<VectorFloat32Parameter>(k_Translation_Key, "Translation", "A pure translation vector", std::vector<float32>{0.0F, 0.0F, 0.0F}, std::vector<std::string>{"X", "Y", "Z"}));
  ...
  params.insert(std::make_unique<ArraySelectionParameter>(k_ComputedTransformationMatrix_Key, "Precomputed Transformation Matrix Path", "A precomputed 4x4 transformation matrix", DataPath{}, ArraySelectionParameter::AllowedTypes{DataType::float32}));
  ...
  params.insertLinkableParameter(std::make_unique<ChoicesParameter>(k_InterpolationType_Key, "Resampling or Interpolation", "Select the type of interpolation algorithm", k_NearestNeighborInterpolationIdx, k_InterpolationChoices));
