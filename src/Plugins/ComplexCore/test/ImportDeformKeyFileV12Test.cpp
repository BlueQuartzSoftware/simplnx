...
TEST_CASE("ComplexCore::ImportDeformKeyFileV12 Case 0", "[Core][ImportDeformKeyFileV12]")
{
  ...
  DataStructure dataStructure;
  {
    ...
    args.insertOrAssign(ImportDeformKeyFileV12Filter::k_InputFilePath_Key, std::make_any<FileSystemPathParameter::ValueType>(fs::path(fmt::format("{}/6_6_import_deform_12/deform_v12_0.key", unit_test::k_TestFilesDir))));
...
// Write the DataStructure out to the file system
  ...
  WriteTestDataStructure(dataStructure, fs::path(fmt::format("{}/7_0_import_deform_key_v12_test.dream3d", unit_test::k_BinaryTestOutputDir)));
#endif
}
...
TEST_CASE("ComplexCore::ImportDeformKeyFileV12 Case 1", "[Core][ImportDeformKeyFileV12]")
{
  ...
  DataStructure dataStructure;
  {
    ...
    // Create default Parameters for the filter.
    args.insertOrAssign(ImportDeformKeyFileV12Filter::k_InputFilePath_Key, std::make_any<FileSystemPathParameter::ValueType>(fs::path(fmt::format("{}/6_6_import_deform_12/deform_v12_1.key", unit_test::k_TestFilesDir))));
