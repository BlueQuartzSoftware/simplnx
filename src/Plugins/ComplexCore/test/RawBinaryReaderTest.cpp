...
void TestCase1_Execute(NumericType scalarType)
{
  ...
  const DataArray<T>& createdData = dataGraph.getDataRefAs<DataArray<T>>(k_CreatedArrayPath);
...
void TestCase4_Execute(NumericType scalarType)
{
  ...
  DataArray<T>* createdArray = dataGraph.getDataAs<DataArray<T>>(k_CreatedArrayPath);
