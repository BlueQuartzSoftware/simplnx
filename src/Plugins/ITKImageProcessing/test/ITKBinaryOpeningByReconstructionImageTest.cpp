...
TEST_CASE("ITKImageProcessing::ITKBinaryOpeningByReconstructionImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKBinaryOpeningByReconstructionImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKBinaryOpeningByReconstructionImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKBinaryOpeningByReconstructionImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKBinaryOpeningByReconstructionImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<ChoicesParameter::ValueType>(ITKBinaryOpeningByReconstructionImageFilter::k_KernelType_Key) == 0);
      CHECK(args.value<float64>(ITKBinaryOpeningByReconstructionImageFilter::k_ForegroundValue_Key) == 2.5);
      CHECK(args.value<float64>(ITKBinaryOpeningByReconstructionImageFilter::k_BackgroundValue_Key) == 2.5);
      CHECK(args.value<bool>(ITKBinaryOpeningByReconstructionImageFilter::k_FullyConnected_Key) == true);
      // Complex type (UInt32Vec3FilterParameterConverter) - verified by successful pipeline loading
      CHECK(args.value<DataPath>(ITKBinaryOpeningByReconstructionImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKBinaryOpeningByReconstructionImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKBinaryOpeningByReconstructionImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
