...
TEST_CASE("ITKImageProcessing::ITKGradientMagnitudeRecursiveGaussianImageFilter(default)", "[ITKImageProcessing][ITKGradientMagnitudeRecursiveGaussianImage][default]")
{
  ...
  args.insertOrAssign(ITKGradientMagnitudeRecursiveGaussianImage::k_OutputImageDataPath_Key, std::make_any<DataObjectNameParameter::ValueType>(outputArrayName));
  ...
  const fs::path baselineFilePath = fs::path(nx::core::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_GradientMagnitudeRecursiveGaussianImageFilter_default.nrrd";
