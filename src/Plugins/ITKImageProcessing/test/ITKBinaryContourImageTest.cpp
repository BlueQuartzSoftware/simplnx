...
TEST_CASE("ITKImageProcessing::ITKBinaryContourImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKBinaryContourImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKBinaryContourImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKBinaryContourImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKBinaryContourImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<bool>(ITKBinaryContourImageFilter::k_FullyConnected_Key) == true);
      CHECK(args.value<float64>(ITKBinaryContourImageFilter::k_BackgroundValue_Key) == 2.5);
      CHECK(args.value<float64>(ITKBinaryContourImageFilter::k_ForegroundValue_Key) == 2.5);
      CHECK(args.value<DataPath>(ITKBinaryContourImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKBinaryContourImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKBinaryContourImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
