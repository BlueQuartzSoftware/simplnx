...
TEST_CASE("ITKImageProcessing::ITKDiscreteGaussianImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKDiscreteGaussianImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKDiscreteGaussianImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKDiscreteGaussianImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKDiscreteGaussianImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      // Complex type (DoubleVec3FilterParameterConverter) - verified by successful pipeline loading
      CHECK(args.value<uint32>(ITKDiscreteGaussianImageFilter::k_MaximumKernelWidth_Key) == 5);
      // Complex type (DoubleVec3FilterParameterConverter) - verified by successful pipeline loading
      CHECK(args.value<bool>(ITKDiscreteGaussianImageFilter::k_UseImageSpacing_Key) == true);
      CHECK(args.value<DataPath>(ITKDiscreteGaussianImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKDiscreteGaussianImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKDiscreteGaussianImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
