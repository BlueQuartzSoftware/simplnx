...
TEST_CASE("ITKImageProcessing::ITKGrayscaleMorphologicalOpeningImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKGrayscaleMorphologicalOpeningImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKGrayscaleMorphologicalOpeningImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKGrayscaleMorphologicalOpeningImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKGrayscaleMorphologicalOpeningImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<ChoicesParameter::ValueType>(ITKGrayscaleMorphologicalOpeningImageFilter::k_KernelType_Key) == 0);
      CHECK(args.value<bool>(ITKGrayscaleMorphologicalOpeningImageFilter::k_SafeBorder_Key) == true);
      // Complex type (UInt32Vec3FilterParameterConverter) - verified by successful pipeline loading
      CHECK(args.value<DataPath>(ITKGrayscaleMorphologicalOpeningImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKGrayscaleMorphologicalOpeningImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKGrayscaleMorphologicalOpeningImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
