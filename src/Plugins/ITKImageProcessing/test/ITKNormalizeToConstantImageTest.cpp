...
TEST_CASE("ITKImageProcessing::ITKNormalizeToConstantImageFilter(defaults)", "[ITKImageProcessing][ITKNormalizeToConstantImage][defaults]")
{
  DataStructure dataStructure;
  const ITKNormalizeToConstantImageFilter filter;
  ...
  Arguments args;
  args.insertOrAssign(ITKNormalizeToConstantImageFilter::k_InputImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
  args.insertOrAssign(ITKNormalizeToConstantImageFilter::k_InputImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
  args.insertOrAssign(ITKNormalizeToConstantImageFilter::k_OutputImageArrayName_Key, std::make_any<DataObjectNameParameter::ValueType>(outputArrayName));
  ...
  const fs::path baselineFilePath = fs::path(nx::core::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_NormalizeToConstantImageFilter_defaults.nrrd";
...
TEST_CASE("ITKImageProcessing::ITKNormalizeToConstantImageFilter(vector)", "[ITKImageProcessing][ITKNormalizeToConstantImage][vector]")
{
  ...
  const fs::path baselineFilePath = fs::path(nx::core::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_NormalizeToConstantImageFilter_vector.nrrd";
  const DataPath baselineGeometryPath({ITKTestBase::k_BaselineGeometryPath});
  const DataPath baseLineCellDataPath = baselineGeometryPath.createChildPath(ITKTestBase::k_ImageCellDataName);
  const DataPath baselineDataPath = baseLineCellDataPath.createChildPath(ITKTestBase::k_BaselineDataPath);
  const Result<> readBaselineResult = ITKTestBase::ReadImage(dataStructure, baselineFilePath, baselineGeometryPath, ITKTestBase::k_ImageCellDataName, ITKTestBase::k_BaselineDataPath);
