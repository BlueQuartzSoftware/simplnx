...
TEST_CASE("ITKImageProcessing::ITKGrayscaleMorphologicalClosingImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKGrayscaleMorphologicalClosingImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKGrayscaleMorphologicalClosingImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKGrayscaleMorphologicalClosingImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKGrayscaleMorphologicalClosingImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<ChoicesParameter::ValueType>(ITKGrayscaleMorphologicalClosingImageFilter::k_KernelType_Key) == 0);
      CHECK(args.value<bool>(ITKGrayscaleMorphologicalClosingImageFilter::k_SafeBorder_Key) == true);
      // Complex type (UInt32Vec3FilterParameterConverter) - verified by successful pipeline loading
      CHECK(args.value<DataPath>(ITKGrayscaleMorphologicalClosingImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKGrayscaleMorphologicalClosingImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKGrayscaleMorphologicalClosingImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
