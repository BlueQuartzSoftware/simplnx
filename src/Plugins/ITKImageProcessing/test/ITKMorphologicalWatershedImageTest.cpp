...
TEST_CASE("ITKImageProcessing::ITKMorphologicalWatershedImageFilter(defaults)", "[ITKImageProcessing][ITKMorphologicalWatershedImage][defaults]")
{
  ...
  { // Start Image Comparison Scope
    const fs::path inputFilePath = fs::path(nx::core::unit_test::k_SourceDir.view()) / unit_test::k_DataDir.view() / "JSONFilters" / "Input/cthead1-grad-mag.nrrd";
...
TEST_CASE("ITKImageProcessing::ITKMorphologicalWatershedImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKMorphologicalWatershedImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKMorphologicalWatershedImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKMorphologicalWatershedImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKMorphologicalWatershedImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<float64>(ITKMorphologicalWatershedImageFilter::k_Level_Key) == 2.5);
      CHECK(args.value<bool>(ITKMorphologicalWatershedImageFilter::k_MarkWatershedLine_Key) == true);
      CHECK(args.value<bool>(ITKMorphologicalWatershedImageFilter::k_FullyConnected_Key) == true);
      CHECK(args.value<DataPath>(ITKMorphologicalWatershedImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKMorphologicalWatershedImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKMorphologicalWatershedImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
