...
TEST_CASE("ITKImageProcessing::ITKHMaximaImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKHMaximaImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKHMaximaImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKHMaximaImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKHMaximaImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<float64>(ITKHMaximaImageFilter::k_Height_Key) == 2.5);
      CHECK(args.value<DataPath>(ITKHMaximaImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKHMaximaImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKHMaximaImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
