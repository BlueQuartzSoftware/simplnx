...
TEST_CASE("ITKImageProcessing::ITKBoundedReciprocalImageFilter(defaults)", "[ITKImageProcessing][ITKBoundedReciprocalImage][defaults]")
{
  ...
  Arguments args;
  args.insertOrAssign(ITKBoundedReciprocalImage::k_InputImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
  args.insertOrAssign(ITKBoundedReciprocalImage::k_InputImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
  args.insertOrAssign(ITKBoundedReciprocalImage::k_OutputImageArrayName_Key, std::make_any<DataObjectNameParameter::ValueType>(outputArrayName));
  ...
  const fs::path baselineFilePath = fs::path(nx::core::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_BoundedReciprocalImageFilter_defaults.nrrd";
...
TEST_CASE("ITKImageProcessing::ITKBoundedReciprocalImageFilter(vector)", "[ITKImageProcessing][ITKBoundedReciprocalImage][vector]")
{
  ...
  const fs::path baselineFilePath = fs::path(nx::core::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_BoundedReciprocalImageFilter_vector.nrrd";
  const DataPath baselineGeometryPath({ITKTestBase::k_BaselineGeometryPath});
  const DataPath baseLineCellDataPath = baselineGeometryPath.createChildPath(ITKTestBase::k_ImageCellDataName);
  const DataPath baselineDataPath = baseLineCellDataPath.createChildPath(ITKTestBase::k_BaselineDataPath);
  const Result<> readBaselineResult = ITKTestBase::ReadImage(dataStructure, baselineFilePath, baselineGeometryPath, ITKTestBase::k_ImageCellDataName, ITKTestBase::k_BaselineDataPath);
