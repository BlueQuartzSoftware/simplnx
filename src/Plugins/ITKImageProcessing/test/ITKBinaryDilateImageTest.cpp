...
TEST_CASE("ITKImageProcessing::ITKBinaryDilateImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKBinaryDilateImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKBinaryDilateImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKBinaryDilateImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKBinaryDilateImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<ChoicesParameter::ValueType>(ITKBinaryDilateImageFilter::k_KernelType_Key) == 0);
      CHECK(args.value<float64>(ITKBinaryDilateImageFilter::k_BackgroundValue_Key) == 2.5);
      CHECK(args.value<float64>(ITKBinaryDilateImageFilter::k_ForegroundValue_Key) == 2.5);
      CHECK(args.value<bool>(ITKBinaryDilateImageFilter::k_BoundaryToForeground_Key) == true);
      // Complex type (UInt32Vec3FilterParameterConverter) - verified by successful pipeline loading
      CHECK(args.value<DataPath>(ITKBinaryDilateImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKBinaryDilateImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKBinaryDilateImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
