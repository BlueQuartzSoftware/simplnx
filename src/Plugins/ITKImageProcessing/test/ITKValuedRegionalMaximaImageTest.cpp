...
TEST_CASE("ITKImageProcessing::ITKValuedRegionalMaximaImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKValuedRegionalMaximaImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKValuedRegionalMaximaImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKValuedRegionalMaximaImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKValuedRegionalMaximaImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<bool>(ITKValuedRegionalMaximaImageFilter::k_FullyConnected_Key) == true);
      CHECK(args.value<DataPath>(ITKValuedRegionalMaximaImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKValuedRegionalMaximaImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKValuedRegionalMaximaImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
