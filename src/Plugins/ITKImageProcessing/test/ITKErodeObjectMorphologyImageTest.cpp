...
TEST_CASE("ITKImageProcessing::ITKErodeObjectMorphologyImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKErodeObjectMorphologyImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKErodeObjectMorphologyImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKErodeObjectMorphologyImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKErodeObjectMorphologyImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<ChoicesParameter::ValueType>(ITKErodeObjectMorphologyImageFilter::k_KernelType_Key) == 0);
      CHECK(args.value<float64>(ITKErodeObjectMorphologyImageFilter::k_ObjectValue_Key) == 2.5);
      CHECK(args.value<float64>(ITKErodeObjectMorphologyImageFilter::k_BackgroundValue_Key) == 2.5);
      // Complex type (UInt32Vec3FilterParameterConverter) - verified by successful pipeline loading
      CHECK(args.value<DataPath>(ITKErodeObjectMorphologyImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKErodeObjectMorphologyImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKErodeObjectMorphologyImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
