...
TEST_CASE("ITKImageProcessing::ITKSignedDanielssonDistanceMapImageFilter(default)", "[ITKImageProcessing][ITKSignedDanielssonDistanceMapImage][default]")
{
  ...
  Arguments args;
  args.insertOrAssign(ITKSignedDanielssonDistanceMapImage::k_InputImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
  args.insertOrAssign(ITKSignedDanielssonDistanceMapImage::k_InputImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
  ...
  const fs::path baselineFilePath = fs::path(nx::core::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_SignedDanielssonDistanceMapImageFilter_default.nrrd";
