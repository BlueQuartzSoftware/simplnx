...
TEST_CASE("ITKImageProcessing::ITKGrayscaleGrindPeakImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKGrayscaleGrindPeakImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKGrayscaleGrindPeakImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKGrayscaleGrindPeakImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKGrayscaleGrindPeakImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<bool>(ITKGrayscaleGrindPeakImageFilter::k_FullyConnected_Key) == true);
      CHECK(args.value<DataPath>(ITKGrayscaleGrindPeakImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKGrayscaleGrindPeakImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKGrayscaleGrindPeakImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
