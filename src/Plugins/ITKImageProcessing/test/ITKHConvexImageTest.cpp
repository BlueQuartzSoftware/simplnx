...
TEST_CASE("ITKImageProcessing::ITKHConvexImageFilter: SIMPL Backwards Compatibility", "[ITKImageProcessing][ITKHConvexImageFilter][BackwardsCompatibility]")
{
  ...
  const std::vector<std::pair<std::string, fs::path>> fixtures = {
      {"SIMPL 6.5 (UUID)", conversionDir / "6_5" / "ITKHConvexImageFilter.json"},
      {"SIMPL 6.4 (Filter_Name)", conversionDir / "6_4" / "ITKHConvexImageFilter.json"},
  };
  ...
  for(const auto& [label, fixturePath] : fixtures)
  {
    DYNAMIC_SECTION(label)
    {
      ...
      REQUIRE(filter->uuid() == FilterTraits<ITKHConvexImageFilter>::uuid);

      CHECK(pipelineFilter->getComments().empty());

      const Arguments args = pipelineFilter->getArguments();
      CHECK(args.value<float64>(ITKHConvexImageFilter::k_Height_Key) == 2.5);
      CHECK(args.value<bool>(ITKHConvexImageFilter::k_FullyConnected_Key) == true);
      CHECK(args.value<DataPath>(ITKHConvexImageFilter::k_InputImageGeomPath_Key) == DataPath({"DataContainer"}));
      CHECK(args.value<DataPath>(ITKHConvexImageFilter::k_InputImageDataPath_Key) == DataPath({"DataContainer", "CellData", "TestArray"}));
      CHECK(args.value<std::string>(ITKHConvexImageFilter::k_OutputImageArrayName_Key) == "TestName");
    }
  }
}
