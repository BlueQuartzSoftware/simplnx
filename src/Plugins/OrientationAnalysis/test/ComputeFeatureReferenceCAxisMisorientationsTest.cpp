...
//
// The retired caxis_data exemplar was a circular oracle generated by SIMPLNX.
// Four hand-derived fixtures cover all seven algorithm paths and a Class 4 invariant sweep.
// The realistic fixture exercises the all-non-hexagonal Feature-to-NaN path.
//
...
// A pure Bunge ZXZ rotation (0, phiDeg, 0) rotates about the x-axis.
// The crystal c-axis becomes [0, sin(phiDeg), cos(phiDeg)] in the sample frame.
// Cell-to-average c-axis misorientation is |phi_cell - phi_avg|, folded into [0, 90] degrees.
...
// This function builds an ImageGeom-backed scaffold. Cell arrays use the {nZ, nY, nX} shape.
// AvgCAxes uses {numFeatures}. CrystalStructures uses {numCrystalStructures}.
// Each cell defaults to feature 1, phase 1, and the identity quaternion.
// Each average c axis defaults to sample Z. CrystalStructures index 0 contains the 999 sentinel.
FixtureData CreateScaffold(usize nX, usize nY, usize nZ, usize numFeatures, usize numCrystalStructures)
{
  ...
  td.quats = Float32Array::Create(td.ds, k_QuatsName, quatsStore, td.cellAM->getId());
  auto avgCAxesStore = DataStoreUtilities::CreateDataStore<float32>(td.ds, k_FeatureDataPath.createChildPath(k_AvgCAxesName), {numFeatures}, {3});
  td.avgCAxes = Float32Array::Create(td.ds, k_AvgCAxesName, avgCAxesStore, td.featureAM->getId());
...
{
  REQUIRE_NOTHROW(ds.getDataRefAs<Float32Array>(k_CellDataPath.createChildPath(k_FRCAxisMisOutName)));
...
{
  REQUIRE_NOTHROW(ds.getDataRefAs<Float32Array>(k_FeatureDataPath.createChildPath(k_FeatAvgCAxisMisOutName)));
...
{
  REQUIRE_NOTHROW(ds.getDataRefAs<Float32Array>(k_FeatureDataPath.createChildPath(k_FeatStdevCAxisMisOutName)));
...
TEST_CASE("OrientationAnalysis::ComputeFeatureReferenceCAxisMisorientationsFilter: Phase Index Bounds", "[OrientationAnalysis][ComputeFeatureReferenceCAxisMisorientationsFilter]")
{
  ...
  AnalyticalFixtures::FixtureData fixture = AnalyticalFixtures::CreateScaffold(/*nX=*/2, /*nY=*/1, /*nZ=*/1, /*numFeatures=*/2, /*numCrystalStructures=*/2);
  ...
  ComputeFeatureReferenceCAxisMisorientationsFilter filter;
  Arguments args = AnalyticalFixtures::BuildArgs();
  ...
  SECTION("Participating Phase returns an error")
  {
    ...
    SIMPLNX_RESULT_REQUIRE_INVALID(executeResult.result);
    REQUIRE(executeResult.result.errors()[0].code == -9804);
  }
