...
{
  UnitTest::LoadPlugins();
  ToyFixtures::ToyData td = ToyFixtures::CreateScaffold(2, 2, 2, 2);
  ...
  Arguments args = ToyFixtures::BuildArgs(0);
  ...
  for(usize i = 0; i < 8; ++i)
  {
    ToyFixtures::RequireFRMClose(td.ds, i, 0.0f);
  }
  ToyFixtures::RequireAvgClose(td.ds, 1, 0.0f);
...
TEST_CASE("OrientationAnalysis::ComputeFeatureReferenceMisorientationsFilter: Class 1 - Mode 0 KnownAngle5deg", "[OrientationAnalysis][ComputeFeatureReferenceMisorientationsFilter]")
{
  ...
  const auto qVoxel = ToyFixtures::QuatFromPhi1Deg(5.0f);
  for(usize i = 0; i < 8; ++i)
  {
    (*td.featureIds)[i] = 1;
    (*td.cellPhases)[i] = 1;
  ...
  for(usize i = 0; i < 8; ++i)
  {
    ToyFixtures::RequireFRMClose(td.ds, i, 5.0f);
  }
  ToyFixtures::RequireAvgClose(td.ds, 1, 5.0f);
  ToyFixtures::AssertClass4Invariants(td.ds, /*isMode1=*/false);
}
...
{
  UnitTest::LoadPlugins();
  ToyFixtures::ToyData td = ToyFixtures::CreateScaffold(4, 3, 1, 5);

  const auto qIdentity = ToyFixtures::QuatFromPhi1Deg(0.0f);
  const auto q5 = ToyFixtures::QuatFromPhi1Deg(5.0f);
  const auto q10 = ToyFixtures::QuatFromPhi1Deg(10.0f);
  ...
  ToyFixtures::RequireFRMClose(td.ds, 0, 0.0f); // background
  ToyFixtures::RequireFRMClose(td.ds, 1, 5.0f); // feature 1
  ToyFixtures::RequireFRMClose(td.ds, 2, 5.0f);
  ToyFixtures::RequireFRMClose(td.ds, 3, 0.0f); // feature 2
  ToyFixtures::RequireFRMClose(td.ds, 4, 0.0f);
  ToyFixtures::RequireFRMClose(td.ds, 5, 5.0f); // feature 3, only valid voxel
  ToyFixtures::RequireFRMClose(td.ds, 6, 0.0f); // feature 3, un-phased
  ToyFixtures::RequireFRMClose(td.ds, 7, 0.0f);
  ToyFixtures::RequireFRMClose(td.ds, 8, 0.0f); // feature 4, all un-phased
  ToyFixtures::RequireFRMClose(td.ds, 9, 0.0f);
  ToyFixtures::RequireFRMClose(td.ds, 10, 0.0f);
  ToyFixtures::RequireFRMClose(td.ds, 11, 0.0f);

  // Per-feature expected averages.
  ToyFixtures::RequireAvgClose(td.ds, 0, 0.0f); // background (no voxels contribute)
  ToyFixtures::RequireAvgClose(td.ds, 1, 5.0f); // (5 + 5) / 2
  ToyFixtures::RequireAvgClose(td.ds, 2, 0.0f); // (0 + 0) / 2
  ToyFixtures::RequireAvgClose(td.ds, 3, 5.0f); // 5 / 1 (only voxel 5 valid)
  ToyFixtures::RequireAvgClose(td.ds, 4, 0.0f); // count==0 -> avg=0 (path 7)
...
{
  UnitTest::LoadPlugins();
  ToyFixtures::ToyData td = ToyFixtures::CreateScaffold(3, 3, 1, 2);
  ...
  ComputeFeatureReferenceMisorientationsFilter filter;
  Arguments args = ToyFixtures::BuildArgs(1);
  auto preflightResult = filter.preflight(td.ds, args);
  SIMPLNX_RESULT_REQUIRE_VALID(preflightResult.outputActions);
  auto executeResult = filter.execute(td.ds, args);
  SIMPLNX_RESULT_REQUIRE_VALID(executeResult.result);
  ...
  for(usize i = 0; i < 9; ++i)
  {
    ToyFixtures::RequireFRMClose(td.ds, i, (i == 4) ? 0.0f : 5.0f);
  }
  // avg = (0 + 5*8) / 9 = 40/9 deg
  ToyFixtures::RequireAvgClose(td.ds, 1, 40.0f / 9.0f);

  // EuclideanCenters[1] should be coords of voxel 4 = (1.5, 1.5, 0.5) with spacing 1 and origin 0.
  const auto& centers = td.ds.getDataRefAs<Float32Array>(ToyFixtures::k_CellFeatureDataPath.createChildPath(ToyFixtures::k_FeatureEuclideanCentersOutName));
  ...
  ToyFixtures::AssertClass4Invariants(td.ds, /*isMode1=*/true);
}
...
{
  UnitTest::LoadPlugins();
  ToyFixtures::ToyData td = ToyFixtures::CreateScaffold(2, 3, 1, 3);
  ...
  ToyFixtures::RequireFRMClose(td.ds, 0, 5.0f);
  ToyFixtures::RequireFRMClose(td.ds, 1, 0.0f);
  ToyFixtures::RequireFRMClose(td.ds, 2, 5.0f);
  ToyFixtures::RequireFRMClose(td.ds, 3, 5.0f);
  ToyFixtures::RequireFRMClose(td.ds, 4, 5.0f);
  ToyFixtures::RequireFRMClose(td.ds, 5, 0.0f);
  // avg[1] = (5+0+5)/3 = 10/3; avg[2] = (5+5+0)/3 = 10/3.
  ToyFixtures::RequireAvgClose(td.ds, 1, 10.0f / 3.0f);
  ToyFixtures::RequireAvgClose(td.ds, 2, 10.0f / 3.0f);
...
{
  UnitTest::LoadPlugins();
  ToyFixtures::ToyData td = ToyFixtures::CreateScaffold(3, 3, 2, 2);
  ...
  for(usize i = 0; i < 18; ++i)
  {
    ToyFixtures::RequireFRMClose(td.ds, i, (i == 13) ? 0.0f : 5.0f);
  }
  // avg = (0 + 5*17) / 18 = 85/18 deg.
  ToyFixtures::RequireAvgClose(td.ds, 1, 85.0f / 18.0f);
...
TEST_CASE("OrientationAnalysis::ComputeFeatureReferenceMisorientationsFilter: Class 4 - Invariants Sweep", "[OrientationAnalysis][ComputeFeatureReferenceMisorientationsFilter]")
{
  ...
  {
    ToyFixtures::ToyData td = ToyFixtures::CreateScaffold(3, 3, 1, 3);
    const auto q0 = ToyFixtures::QuatFromPhi1Deg(0.0f);
    const auto q7 = ToyFixtures::QuatFromPhi1Deg(7.0f);
