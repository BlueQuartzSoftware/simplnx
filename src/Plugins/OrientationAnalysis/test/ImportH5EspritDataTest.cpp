...
TEST_CASE("OrientationAnalysis::ImportH5EspritDataFilter: Valid Filter Execution", "[OrientationAnalysis][ImportH5EspritDataFilter]")
{
  ...
  args.insertOrAssign(ImportH5EspritDataFilter::k_SelectedScanNames_Key, std::make_any<OEMEbsdScanSelectionParameter::ValueType>(scanSelections));
  args.insertOrAssign(ImportH5EspritDataFilter::k_ZSpacing_Key, std::make_any<float32>(1.0f));
  args.insertOrAssign(ImportH5EspritDataFilter::k_Origin_Key, std::make_any<VectorFloat32Parameter::ValueType>(std::vector<float32>(3, 0.0f)));
  args.insertOrAssign(ImportH5EspritDataFilter::k_DegreesToRadians_Key, std::make_any<bool>(true));
  args.insertOrAssign(ImportH5EspritDataFilter::k_ReadPatternData_Key, std::make_any<bool>(false));
  args.insertOrAssign(ImportH5EspritDataFilter::k_ImageGeometryName_Key, std::make_any<DataPath>(DataPath({ImageGeom::k_TypeName})));
  args.insertOrAssign(ImportH5EspritDataFilter::k_CellAttributeMatrixName_Key, std::make_any<std::string>(k_CellData));
  args.insertOrAssign(ImportH5EspritDataFilter::k_CellEnsembleAttributeMatrixName_Key, std::make_any<std::string>(k_CellEnsembleData));
...
TEST_CASE("OrientationAnalysis::ImportH5EspritDataFilter: InValid Filter Execution", "[OrientationAnalysis][ImportH5EspritDataFilter]")
{
  ...
  {
    args.insertOrAssign(ImportH5EspritDataFilter::k_SelectedScanNames_Key, std::make_any<OEMEbsdScanSelectionParameter::ValueType>(scanSelections));
    args.insertOrAssign(ImportH5EspritDataFilter::k_ZSpacing_Key, std::make_any<float32>(0.0f));
    args.insertOrAssign(ImportH5EspritDataFilter::k_ReadPatternData_Key, std::make_any<bool>(false));
  }
  ...
  SECTION("Pattern data doesn't exist error")
  {
    args.insertOrAssign(ImportH5EspritDataFilter::k_SelectedScanNames_Key, std::make_any<OEMEbsdScanSelectionParameter::ValueType>(scanSelections));
    args.insertOrAssign(ImportH5EspritDataFilter::k_ZSpacing_Key, std::make_any<float32>(1.0f));
    args.insertOrAssign(ImportH5EspritDataFilter::k_ReadPatternData_Key, std::make_any<bool>(true));
  }
