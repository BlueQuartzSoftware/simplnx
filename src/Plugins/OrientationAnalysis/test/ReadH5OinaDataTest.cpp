 ...
 *                          raw file bytes, bypassing H5OINAReader entirely. The
 *                          equivalent h5py readback is
 *                          ww_work/ReadH5OinaData/h5oina_oracle.py (readback mode);
 *                          its recorded output is readback_real_file.txt.
 *
 * Precision pinning (the ReadCtfData lesson): fixture values are float32-exact.
 * The hexagonal alignment adds 30 degrees expressed in radians, computed with a
 * double-precision intermediate and stored back as float32. Three fixture phi2
 * values (0.1F, 0.34F, and Fixture C's 0.09F/0.22F/0.35F) are chosen because
 * their correctly-rounded float32 results DIFFER between a double-precision
 * intermediate and a float32 intermediate, so the expectations pin the shape of
 * the arithmetic and not merely its magnitude. 0.25F is carried alongside as a
 * dyadic control whose result is identical under either intermediate. Every such
 * literal was derived with IEEE-754 float32/float64 semantics in NumPy; the
 * derivation lives in ww_work/ReadH5OinaData/h5oina_oracle.py and its recorded
 * output oracle_spec.txt.
 *
...
//------------------------------------------------------------------------------
// Toy .h5oina fixture description. The dataset set below is exactly the minimum
// H5OINAReader requires: the four Header scalars, one or more Phases/<n> groups
// carrying Phase Name / Lattice Dimensions / Lattice Angles / Laue Group /
// Space Group, and the nine Data datasets it reads unconditionally. The root
// Manufacturer / Software Version / Index datasets are written for realism only
// -- the reader never reads them.
//------------------------------------------------------------------------------
struct PhaseSpec
{
  std::string name;
...
{
  const hid_t fileId = H5Utilities::openFile(filePath.string(), true);
  REQUIRE(fileId >= 0);
  std::vector<T> data;
  const herr_t err = H5Lite::readVectorDataset<T>(fileId, datasetPath, data);
  REQUIRE(err >= 0);
  REQUIRE(H5Utilities::closeFile(const_cast<hid_t&>(fileId)) >= 0);
  return data;
}
} // namespace
...
TEST_CASE("OrientationAnalysis::ReadH5OinaDataFilter: Multi-Scan Slab Placement", "[OrientationAnalysis][ReadH5OinaDataFilter]")
{
  ...
  CompareArrayValues<int32>(dataStructure, k_CellAMPath.createChildPath(ebsdlib::H5OINA::Phase), {1, 1, 1, 1, 1, 1, 1, 1});
...
TEST_CASE("OrientationAnalysis::ReadH5OinaDataFilter: Phase Index Out Of Range rejected (-9587)", "[OrientationAnalysis][ReadH5OinaDataFilter]")
{
  ...
  const fs::path inputFile = WriteH5OinaFixture("read_h5oina_vv_phase_index.h5oina", {scan});
...
TEST_CASE("OrientationAnalysis::ReadH5OinaDataFilter: EbsdLib Error Passthrough - Missing Lattice Angles (-9582)", "[OrientationAnalysis][ReadH5OinaDataFilter]")
{
  ...
  const fs::path inputFile = WriteH5OinaFixture("read_h5oina_vv_missing_angles.h5oina", {scan});
  ...
  REQUIRE(preflightResult.outputActions.errors()[0].message.find("'1'") != std::string::npos);
