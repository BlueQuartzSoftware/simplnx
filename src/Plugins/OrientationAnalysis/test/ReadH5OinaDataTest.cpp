 ...
 *
 * Precision pinning (the ReadCtfData lesson): every value written into a toy
 * fixture below is a float32 literal stored as float32, so the file round-trips it
 * bit-for-bit and the verbatim copies are asserted with exact equality. (The
 * production file's values are whatever AZtec wrote; that test asserts exact
 * equality too, but against an independent readback of the file rather than
 * against literals.)
 *
 ...
 * results are identical under either intermediate. Every such literal was derived
 * with IEEE-754 float32/float64 semantics in NumPy; the derivation lives in
 * ww_work/ReadH5OinaData/h5oina_oracle.py and its recorded output oracle_spec.txt.
 *
