...
- **`ComputeFeatureNeighborCAxisMisalignmentsFilter-D1`** — Divisor bug (fixed in this V&V cycle on SIMPLNX side; root cause proven by applying the same fix to a local build of the legacy source). Production-relevant via shipping `EBSD_Hexagonal_Data_Analysis.d3dpipeline`.
