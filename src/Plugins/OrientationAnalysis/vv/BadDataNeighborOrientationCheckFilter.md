...
10. **Algorithm review hardening (Phase 7 of this V&V cycle)**: cancel checking added at all loop levels; `getDataAs<ImageGeom>` replaced with `getDataRefAs<ImageGeom>`; CrystalStructures bounds-validation at `operator()` entry; defensive per-voxel `laueClassIndex` guard; tightened naming + comments. No behavioral delta.
