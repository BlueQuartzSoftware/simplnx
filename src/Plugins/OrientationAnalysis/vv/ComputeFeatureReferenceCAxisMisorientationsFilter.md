...
|------------------------|--------------------------|
| Algorithm Relationship | **Port with Minor Changes** — same per-cell c-axis projection + per-feature avg/stddev finalize as legacy `FindFeatureReferenceCAxisMisorientations`. PR #1438 added the `isHex` gate (skip non-hex cells, closes legacy D1 bug) + Eigen-based math (closes D2 precision drift); PR #1472 swapped EbsdLib 2.0 API; PR #1582 added 4 cancel checks. |
...
| Code paths enumerated | 10 of 11 exercised. Cancellation remains outside this regression. |
| Tests today | 8 registered test cases plus 1 hidden HDF5 slice-boundary test. The current inventory includes tuple validation and phase bounds. |
...
The branch retains the non-hexagonal input correction.

| Deviation | Defect | Affected released versions | Resolution in this branch |
|-----------|--------|----------------------------|---------------------------|
| `ComputeFeatureReferenceCAxisMisorientationsFilter-D1` | Non-hexagonal cells contributed invalid c-axis results. | DREAM.3D 6.5.171; DREAM3D-NX releases before the correction in PR #1438. | Skip non-hexagonal cells and write NaN for features with no valid contributors. |

## Code path coverage

**10 of 11 paths exercised** by the V&V test suite. The cancellation path (#8) is structurally present and reviewed but not exercised by an injected cancel signal in the fixtures.

Source: `src/Plugins/OrientationAnalysis/src/OrientationAnalysis/Filters/Algorithms/ComputeFeatureReferenceCAxisMisorientations.cpp` (277 lines).
...
| 5  | (c) Finalize    | Per-feature average `sum / counts` for `featureId ∈ [1, totalFeatures)`. When `counts == 0`, the current implementation writes explicit quiet NaN. (line 176)        | All 4 Class 1 fixtures; `Class 4 — Invariants` sub-section (iii) asserts `std::isnan(featAvg[F3])` for the all-non-hex feature. |
...
| 7  | (c) Finalize    | Per-feature stddev finalize: `sqrt(sumSqDiff / counts)`. When `counts == 0`, the current implementation writes explicit quiet NaN. (line 202)            | All 4 Class 1 fixtures; `Class 4 — Invariants` sub-section (iii) asserts `std::isnan(featStdev[F3])`.          |
...
| 9 | Slice I/O | Accumulate one feature across slices and reread its cell outputs for variance | `genuine HDF5 slice-boundary oracle` — 10°, 20°, 30°; mean 20° and population standard deviation sqrt(200/3) |
| 10 | Validation | Invalid participating cell phase → -9804 | `Phase Index Bounds` |
| 11 | Preflight | Cell array tuple mismatch → -9800 | `Preflight Error - Cell array tuple count mismatch (-9800)` |
...
- Current OOC recertification: **8 registered test cases plus 1 hidden OOC contract test**.

| Test case       | Status               | Notes |
|--------------------------------------------------------------------|----------------------|-------|
| `Valid Filter Execution`         | retired           | Circular oracle — consumes `caxis_data.tar.gz` whose `FeatureReferenceCAxisMisorientations` / `FeatureAvgCAxisMisorientations` / `FeatureStdevCAxisMisorientations` arrays were generated from a SIMPLNX run (or a pre-EbsdLib-2.4.1 SIMPLNX run), not from an independent oracle. Per V&V policy line 33, "matches SIMPLNX-then" is not a correctness check. **Archive download stays in `test/CMakeLists.txt`** — `caxis_data.tar.gz` is also consumed by `ComputeCAxisLocationsTest.cpp` (lines 27, 61). |
| `InValid Filter Execution`       | kept             | Exercises Path 1 (all-non-hex preflight error `-9802`). Mutates `CrystalStructures[1] = 1` (Cubic_High) and asserts execute fails. Re-use unchanged. |
| `SIMPL Backwards Compatibility`  | kept             | DYNAMIC_SECTION over 6.4 (Filter_Name) + 6.5 (UUID) conversion fixtures at `test/simpl_conversion/6_{4,5}/`. Re-use unchanged. |
| `Class 1 — Simple Hex Triple`    | new-for-V&V              | Minimal closed-form fixture. 3×1×1 ImageGeom, 1 hex feature (sentinel + F1), cells at `Φ = 0°, 5°, 10°` (Bunge ZXZ `(0, Φ, 0)`). `AvgCAxes[F1]` set to the pre-computed c-axis at `Φ = 5°` (the geometric mean). Expected: `cellRefCAxisMis = [5°, 0°, 5°]`, `featAvg[F1] = 10/3 ≈ 3.333°`, `featStdev[F1] = √(50/9) ≈ 2.357°`. Exercises Paths 3, 5, 6, 7. |
| `Class 1 — Realistic Microstructure (exposes divide-by-zero)`      | new-for-V&V              | The meaty fixture. 5×5×1 ImageGeom, 6 features (sentinel + 5 real): F1 hex (Φ all 0°), F2 hex (Φ = 8,9,10,11,12°), F3 cubic, F4 hex (Φ all 20°), F5 hex (Φ = 25,28,30,32,35°). Per-feature cell spreads chosen for non-trivial stddev. **F3 is the load-bearing feature**: 0 hex cells → `counts[F3] = 0` → exercises the all-non-hex-feature → NaN path (paths 5, 7) → `featAvg[F3] = NaN`, `featStdev[F3] = NaN` via IEEE 754. Exercises Paths 2, 3, 4, 5, 6, 7. |
| `Class 1 — All-Identical Orientation Feature`     | new-for-V&V              | Invariant I5 + minimal stddev=0 confirmation. 5×1×1, 1 hex feature, 5 cells all at `Φ = 10°`, `AvgCAxes[F1]` = c-axis at `Φ = 10°`. Expected: `cellRefCAxisMis = [0°, 0°, 0°, 0°, 0°]`, `featAvg[F1] = 0°`, `featStdev[F1] = 0°`. |
| `Class 4 — Invariants` (3 SECTIONs)               | new-for-V&V              | Reuses the Realistic Microstructure fixture data. Sub-sections: **(i) Range** — every `cellRefCAxisMis[i] ∈ [0°, 90°]` for hex cells, `== 0.0f` for non-hex cells (invariants I1 + I2). **(ii) Per-feature averaging formula** — `featAvg[f]` equals the arithmetic mean of `cellRefCAxisMis[hex+valid cells in f]` (invariant I3). **(iii) All-non-hex feature → NaN** — `featAvg[F3]` and `featStdev[F3]` are both NaN (invariant I6 — **load-bearing for the V&V finding** at paths 5, 7). |
| `Preflight Error - Cell array tuple count mismatch (-9800)` | kept | Rejects unequal cell tuple counts. |
| `Phase Index Bounds` | kept | Checks participating phase bounds and ignored feature-zero cases. |
| `genuine HDF5 slice-boundary oracle` | new-for-V&V | 17×19×4 cells. The algorithm uses 323-cell slice buffers. Contributors on either side of a slice boundary and in the last slice determine all three output arrays. This proves an algorithm block boundary; the physical HDF5 chunk contains the whole small volume. |


The hidden test reuses `CreateScaffold`, `SetCellQuat`, and the output helpers. Expected values are analytical: [10°, 20°, 30°], mean 20°, and population standard deviation sqrt(200/3), within 0.001°. It also checks that skipped cells do not retain values from a previous slice. The upstream oracle assertions remain intact.
...
- **`ComputeFeatureReferenceCAxisMisorientationsFilter-D2`** — Precision-class drift `~1e-4°` per cell / `~1e-5°` per feature avg between 6.5.171 (hand-rolled MatrixMath + float32 stddev) and SIMPLNX (Eigen + double stddev). Root cause proven by applying the Eigen + double-precision-stddev fixes to a local build of the legacy source. **Recommendation: trust SIMPLNX.** See `vv/deviations/ComputeFeatureReferenceCAxisMisorientationsFilter.md`.

For both D1 and D2 the root cause was proven by patching a local build of the legacy source — empirical A/B confirms byte-identical output between the patched legacy build and SIMPLNX. No additional legacy-side action required from this V&V cycle; contact the DREAM3D team for the legacy-parity patch.
