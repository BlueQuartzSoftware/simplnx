...
|------------------------|------------------------------------------------------------------------------------------------------------------------------|
| Algorithm Relationship | **Rewrite** (plumbing) under the retained SIMPL UUID. Legacy built a vector of 7 `OrientationConverter<T>` subclasses and routed every conversion **input→quaternion→output**; SIMPLNX uses an 8×8 `switch` dispatching to macro-generated convertors that call EbsdLib 2.0 `input.toX()` directly. Two scope deltas: SIMPLNX **adds Stereographic** (8th type) and **restricts input to float32** (legacy accepted float and double). |
...
| Code paths enumerated  | 11 enumerated; **9 of 11 exercised**. Gaps: `-67003` multi-dim guard and the per-tuple cancel branch (low value). 16 dispatch arms (8 same-type + 8 `Unknown`) are unreachable through the filter. |
...
| Exemplar archive       | None — values are inline dispatch landmarks in the test source. Unknown-provenance `k_InitValues` **retired** and replaced by EbsdLib-3.0.0-derived values (cross-checked: seed-0 quaternion == EbsdLib `OrientationConverterTest` exemplar; stereographic == closed form). |
| Legacy comparison      | **Run** (toy fixture, 6 shared eu→X conversions via 6.5.171 PipelineRunner vs nxrunner on byte-identical Euler input). Headline: 4 of 6 bit-identical; max \|Δ\| = **1.78e-6** (cubochoric). 4 deviations: D1 routing/precision (measured), D2 float64 scope, D3 Stereographic-added, D4 error-code surface. See `comparisons/ConvertOrientationsFilter/results/comparison.md`. |
...
- **Legacy** built a `QVector` of **7** `OrientationConverter<T>` subclass instances (Euler / OM / Quaternion / AxisAngle / Rodrigues / Homochoric / Cubochoric — **no Stereographic**), called `setInputData()` on the input-type converter, then `convertRepresentationTo(outputType)`, which internally **always routes input → quaternion → output**. Supported **both `float` and `double`** input arrays.
...
1. **Dispatch rewrite** — converter-class-hierarchy-through-quaternion → direct `input.toX()` 8×8 switch. No output change for the 7 shared types is *expected*, but the conversion routing differs (legacy forces a quaternion intermediate; SIMPLNX may convert directly), so intermediate round-off can differ — Deviation candidate.
...
- `"Dispatch and striding (8x8 matrix)"` — 56 `DYNAMIC_SECTION`s (every `(in,out)` pair incl. Stereographic), 3 distinct general orientations per multi-tuple input array, exact-value comparison vs `k_Ref` (EbsdLib-3.0.0-derived landmarks) at tol 1e-3, plus output component-count/tuple-count striding assertions.
...
The `k_Ref` landmarks are cross-validated independently of EbsdLib: seed-0's quaternion equals EbsdLib `OrientationConverterTest`'s published exemplar `{-0.2919894…, 0.319372, 0.1502762…, 0.8889099…}`, and every stereographic value equals the closed-form projection of the corresponding quaternion.

*Second-engineer review:* *Pending.*

## Code path coverage

*9 of 11 enumerated paths exercised. The 2 gaps are a low-value preflight guard (multi-dimensional component shape) and the per-tuple cancel branch (requires cancel-signal injection). The 8 same-type dispatch arms and 8 `Type::Unknown` arms are unreachable through the filter (blocked at preflight / range-validated by the `ChoicesParameter`) and are listed as such.*
...
| 3  | (a) Preflight  | input component count ≠ expected for input type → `-67004`                                         | *Not directly tested. The `Invalid preflight` test covers the `ChoicesParameter` range + does-not-exist errors; the `-67004` mismatch path is a follow-up gap.* |
...
- `ConvertOrientationsFilter-D1` — float32 differences (measured ≤ **1.78e-6**, 4 of 6 conversions bit-identical) from legacy's forced quaternion-intermediate routing vs SIMPLNX direct `input.toX()` (order of operations + library).
