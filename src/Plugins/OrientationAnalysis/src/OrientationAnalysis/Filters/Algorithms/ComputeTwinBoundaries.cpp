...
public:
  CalculateTwinBoundaryImpl(float32 angtol, float32 axistol, const Int32AbstractDataStore& faceLabels, const Float64AbstractDataStore& faceNormals, const Float32AbstractDataStore& avgQuats,
                            ...
                            Float32AbstractDataStore& twinBoundaryIncoherence, const std::atomic_bool& shouldCancel)
  : m_AxisTol(axistol)
  , m_AngTol(angtol)
  ...
  void generateWithCoherence(usize start, usize end) const
  {
    int32 feature1 = 0, feature2 = 0;
    Matrix3x3 orientationMatrix = {};
    float64 w = 0.0;
          ...
          for(int32 j = 0; j < nsym; j++)
          {
            ...
            for(int32 k = 0; k < nsym; k++)
            {
              ...
              if(axisdiff111 < m_AxisTol && angdiff60 < m_AngTol)
              {
                ...
                float64 norm1 = n.norm();
                float64 norm2 = s_xstl_norm.norm();
  ...
  {
    if constexpr(FindCoherence)
    {
      generateWithCoherence(range.min(), range.max());
    }
    else
    {
...
Result<> ComputeTwinBoundaries::operator()()
{
  ...
  {
    dataAlg.execute(CalculateTwinBoundaryImpl<true>(angtol, axistol, faceLabels, faceNormals, avgQuats, featurePhases, crystalStructures, twinBoundaries, twinBoundaryIncoherence, m_ShouldCancel));
  }
  else
  {
    dataAlg.execute(CalculateTwinBoundaryImpl<false>(angtol, axistol, faceLabels, faceNormals, avgQuats, featurePhases, crystalStructures, twinBoundaries, twinBoundaryIncoherence, m_ShouldCancel));
  }
