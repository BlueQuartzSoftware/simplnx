...
// -----------------------------------------------------------------------------
int64 EBSDSegmentFeatures::getSeed(int32 gnum, int64 nextSeed) const
{
...
bool EBSDSegmentFeatures::determineGrouping(int64 referencePoint, int64 neighborPoint, int32 gnum) const
{
  ...
  int32 laueClass1 = (*m_CrystalStructures)[(*cellPhases)[referencePoint]];
  int32 laueClass2 = (*m_CrystalStructures)[(*cellPhases)[neighborPoint]];
  ...
  if(featureIds[neighborPoint] == 0 && (m_GoodVoxelsArray == nullptr || neighborPointIsGood))
  {
    ...
    if((*cellPhases)[referencePoint] == (*cellPhases)[neighborPoint])
    {
      ...
      w = static_cast<float32>(axisAngle[3]);
    }
...
void EBSDSegmentFeatures::allocateSliceBuffers(int64 dimX, int64 dimY)
{
  ...
  m_CrystalStructures->getDataStoreRef().copyIntoBuffer(0, nonstd::span<uint32>(m_CrystalStructuresCache.data(), numPhases));
}
...
void EBSDSegmentFeatures::prepareForSlice(int64 iz, int64 dimX, int64 dimY, int64 dimZ)
{
  ...
  quatStore.copyIntoBuffer(static_cast<usize>(baseIndex) * 4, nonstd::span<float32>(m_QuatBuffer.data() + quatSlotOffset, sliceSize * 4));

  // Bulk-read phase IDs for this slice
  AbstractDataStore<int32>& phaseStore = m_CellPhases->getDataStoreRef();
  phaseStore.copyIntoBuffer(static_cast<usize>(baseIndex), nonstd::span<int32>(m_PhaseBuffer.data() + slotOffset, sliceSize));
