...
{
  return rgba(std::min(std::max(that.r, 0.0f), 1.0f), std::min(std::max(that.g, 0.0f), 1.0f), std::min(std::max(that.b, 0.0f), 1.0f), std::min(std::max(that.a, 0.0f), 1.0f));
}
...
void canvas::add_glyph(int glyph, float angular)
{
  ...
  if(contours < 0)
  {
    ...
    for(;;)
    {
      ...
      float d = flags & 8 ? a : flags & 64 ? static_cast<float>(signed_16(face.data, offset + 2)) / 16384.0f : flags & 128 ? static_cast<float>(signed_16(face.data, offset + 6)) / 16384.0f : 1.0f;
...
{
  int bytes = ((text[index] & 0x80) == 0x00 ? 1 : (text[index] & 0xe0) == 0xc0 ? 2 : (text[index] & 0xf0) == 0xe0 ? 3 : (text[index] & 0xf8) == 0xf0 ? 4 : 0);
...
void canvas::text_to_lines(char const* text, xy position, float maximum_width, bool stroking)
{
  ...
  for(int index = 0; text[index];)
  {
    ...
    forward = saved_forward;
    transform(scaling.x, 0.0f, 0.0f, -scaling.y, position.x + static_cast<float>(place) * scaling.x, position.y);
    ...
    if(emit)
    {
      ...
      if(scratch.subpaths[subpath].closed && merge_emit)
      {
        ...
        else
        {
          ...
          std::rotate((lines.points.begin() + static_cast<ptrdiff_t>(merge_point)), (lines.points.end() - static_cast<ptrdiff_t>(count)), lines.points.end());
...
void canvas::add_half_stroke(size_t beginning, size_t ending, bool closed)
{
  ...
  do
  {
    ...
    if(in_length != 0.0f && out_length >= epsilon)
    {
      ...
      else if(line_join == rounded)
      {
        ...
        lines.points.push_back(forward * side_in);
        add_bezier(forward * side_in, forward * (side_in + alpha * half * in_direction), forward * (side_out - alpha * half * out_direction), forward * side_out, -1.0f);
      }
      else
      {
        lines.points.push_back(forward * side_in);
  ...
  else if(line_cap == circle)
  {
    ...
    lines.points.push_back(forward * (point + side));
    add_bezier(forward * (point + side), forward * (point + side + alpha * ahead), forward * (point + ahead + alpha * side), forward * (point + ahead), -1.0f);
    add_bezier(forward * (point + ahead), forward * (point + ahead - alpha * side), forward * (point - side + alpha * ahead), forward * (point - side), -1.0f);
  }
}
...
void canvas::add_runs(xy from, xy to)
{
  ...
  do
  {
    ...
    while(next_x.x < next_y.x)
    {
      ...
      pixel_run piece = {static_cast<unsigned short>(pixel.x), static_cast<unsigned short>(pixel.y), (carry + strip - area) * sign};
    ...
    float area = (mid - pixel.x) * strip;
    pixel_run piece_1 = {static_cast<unsigned short>(pixel.x), static_cast<unsigned short>(pixel.y), (carry + strip - area) * sign};
    pixel_run piece_2 = {static_cast<unsigned short>(pixel.x + 1.0f), static_cast<unsigned short>(pixel.y), area * sign};
...
{
  return (left.y < right.y ? true : left.y > right.y ? false : left.x < right.x ? true : left.x > right.x ? false : fabsf(left.delta) < fabsf(right.delta));
}
...
void canvas::lines_to_runs(xy offset, int padding)
{
  ...
  for(size_t subpath = 0; subpath < lines.subpaths.size(); ++subpath)
  {
    ...
    {
      xy normal = xy(edge == 0 ? 1.0f : edge == 2 ? -1.0f : 0.0f, edge == 1 ? 1.0f : edge == 3 ? -1.0f : 0.0f);
    ...
    for(size_t index = 0; index < last; ++index)
    {
      ...
      xy to = scratch.points[index];
      add_runs(xy(std::min(std::max(from.x, 0.0f), width), std::min(std::max(from.y, 0.0f), height)), xy(std::min(std::max(to.x, 0.0f), width), std::min(std::max(to.y, 0.0f), height)));
    }
  }
  if(runs.empty())
    return;
...
rgba canvas::paint_pixel(xy point, paint_brush const& brush)
{
  ...
  size_t index = static_cast<size_t>(std::upper_bound(brush.stops.begin(), brush.stops.end(), offset) - brush.stops.begin());
  if(index == 0)
...
{
  set_transform(forward.a * a + forward.c * b, forward.b * a + forward.d * b, forward.a * c + forward.c * d, forward.b * c + forward.d * d, forward.a * e + forward.c * f + forward.e,
...
void canvas::set_transform(float a, float b, float c, float d, float e, float f)
{
  ...
  affine_matrix new_inverse = {scaling * d, scaling * -b, scaling * -c, scaling * a, scaling * (c * f - d * e), scaling * (b * e - a * f)};
  forward = new_forward;
  inverse = new_inverse;
}
...
void canvas::set_radial_gradient(brush_type type, float start_x, float start_y, float start_radius, float end_x, float end_y, float end_radius)
{
...
void canvas::set_pattern(brush_type type, unsigned char const* image, int width, int height, int stride, repetition_style repetition)
{
  ...
  for(int y = 0; y < height; ++y)
    for(int x = 0; x < width; ++x)
    {
      ...
      rgba color = rgba(image[index + 0] / 255.0f, image[index + 1] / 255.0f, image[index + 2] / 255.0f, image[index + 3] / 255.0f);
...
void canvas::bezier_curve_to(float control_1_x, float control_1_y, float control_2_x, float control_2_y, float x, float y)
{
  if(path.subpaths.empty())
...
void canvas::arc_to(float vertex_x, float vertex_y, float x, float y, float radius)
{
  ...
  bool reverse = static_cast<int>(floorf((angle_2 - angle_1) / 3.14159265f)) & 1;
...
void canvas::draw_image(unsigned char const* image, int width, int height, int stride, float x, float y, float to_width, float to_height)
{
...
void canvas::get_image_data(unsigned char* image, int width, int height, int stride, int x, int y)
{
  ...
  static float const bayer[][4] = {
      {0.03125f, 0.53125f, 0.15625f, 0.65625f}, {0.78125f, 0.28125f, 0.90625f, 0.40625f}, {0.21875f, 0.71875f, 0.09375f, 0.59375f}, {0.96875f, 0.46875f, 0.84375f, 0.34375f}};
