...
#define ADD_PARAMETER_TRAIT(thing1, thing2)                                                                                                                                                            \
  {                                                                                                                                                                                                    \
    auto uuidResult = complex::Uuid::FromString(thing2);                                                                                                                                               \
...
void GenerateParameterList()
{
  ...
  ADD_PARAMETER_TRAIT(complex.ImportTextDataParameter, "4f6d6a33-48da-427a-8b17-61e07d1d5b45")
...
std::string ExtractRstFilterSummary(const std::filesystem::path& docFilePath)
{
  ...
  {
    if(complex::StringUtilities::starts_with(line, "## Description"))
    {
      extractLines = true;
      continue;
    }
    ...
    if(complex::StringUtilities::starts_with(line, "##"))
    {
      extractLines = false;
    }
    if(extractLines)
  ...
  rstDescription = complex::StringUtilities::replace(rstDescription, "\n", "");
  rstDescription = complex::StringUtilities::trimmed(rstDescription);
