...
def do_tuples_match(data_array: cx.IDataArray, image_geom: cx.ImageGeom) -> bool:
...
def do_dimensions_match(data_array: cx.IDataArray, image_geom: cx.ImageGeom) -> bool:
...
class FunctorBase:
  def __init__(self, input_pixel_type, output_pixel_type, output_data_type: cx.DataType, dimension):
...
def array_switch_func_component_impl(input_pixel_type, output_pixel_type, output_data_type: cx.DataType, dimensions: int, component_options: ArrayComponentOptions, result_type, functor: FunctorBase, n_comp, error_code, *args):
  ...
  return cx.Result([cx.Error(error_code, error_message)])


def array_switch_func_dims_impl(input_pixel_type, output_pixel_type, output_data_type: cx.DataType, component_options: ArrayComponentOptions, result_type, functor: FunctorBase, data_array: cx.IDataArray, image_geom: cx.ImageGeom, error_code, *args):
...
def data_check_impl(input_pixel_type, output_pixel_type, output_data_type: cx.DataType, data_structure: cx.DataStructure, input_array_path: cx.DataPath, image_geom_path: cx.DataPath, output_array_path: cx.DataPath) -> cx.Result:
  ...
  image_geom: cx.ImageGeom = data_structure[image_geom_path]
  data_array: cx.IDataArray = data_structure[input_array_path]
  ...
  if not do_dimensions_match(data_array, image_geom) and not do_tuples_match(data_array, image_geom):
    ...
    return cx.Result([cx.Error(k_ImageGeometryDimensionMismatch, err_message)])
  ...
  if c_dims != input_pixel_dims:
    ...
    return cx.Result([cx.Error(k_ImageComponentDimensionMismatch, err_message)])
  ...
  output_actions.append_action(cx.CreateArrayAction(output_data_type, t_dims, output_pixel_dims, output_array_path, data_store.get_data_format()))

  return output_actions

def array_switch_func(functor: FunctorBase, array_options: ArrayOptions, result_type, data_array: cx.IDataArray, image_geom: cx.ImageGeom, error_code: int, *args) -> cx.Result:
  type = data_array.data_type
  c_options: ArrayComponentOptions = array_options.component_options

  if array_options.type_options.using_int8 and type == cx.DataType.int8:
  ...
  elif array_options.type_options.using_uint8 and type == cx.DataType.uint8:
  ...
  elif array_options.type_options.using_int16 and type == cx.DataType.int16:
  ...
  elif array_options.type_options.using_uint16 and type == cx.DataType.uint16:
  ...
  elif array_options.type_options.using_int32 and type == cx.DataType.int32:
  ...
  elif array_options.type_options.using_uint32 and type == cx.DataType.uint32:
  ...
  elif array_options.type_options.using_int64 and type == cx.DataType.int64:
  ...
  elif array_options.type_options.using_uint64 and type == cx.DataType.uint64:
  ...
  elif array_options.type_options.using_float32 and type == cx.DataType.float32:
    return array_switch_func_dims_impl(itk.F, itk.F, type, c_options, result_type, functor, data_array, image_geom, error_code, *args)
  elif array_options.type_options.using_float64 and type == cx.DataType.float64:
    return array_switch_func_dims_impl(itk.D, itk.D, type, c_options, result_type, functor, data_array, image_geom, error_code, *args)

  return cx.Result([cx.Error(-1000, "Invalid DataType while attempting to execute")])

def data_check(array_options: ArrayOptions, data_structure: cx.DataStructure, input_array_path: cx.DataPath, image_geom_path: cx.DataPath, output_array_path: cx.DataPath) -> cx.IFilter.PreflightResult:
  image_geom = data_structure[image_geom_path]
  input_array = data_structure[input_array_path]

  functor = DataCheckImplFunctor()
  result_type = cx.OutputActions()
