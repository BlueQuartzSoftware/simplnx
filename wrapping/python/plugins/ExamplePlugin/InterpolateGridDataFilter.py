...
class InterpolateGridDataFilter:
  ...
  def uuid(self) -> sx.Uuid:
    return sx.Uuid('20f57375-6a0f-46d9-a0fe-59d18c83fb48')
  ...
  def parameters(self) -> sx.Parameters:
    ...
    params.insert(sx.ChoicesParameter(InterpolateGridDataFilter.INTERPOLATION_METHOD, 'Interpolation Method', 'The method used to interpolate the input data.', InterpolateGridDataFilter.InterpolationMethod.NEAREST.value, ['Nearest', 'Linear', 'Cubic']))
    params.insert_linkable_parameter(sx.BoolParameter(InterpolateGridDataFilter.USE_FILL_VALUE, 'Use Fill Value', 'Determines whether or not to use the Fill Value parameter.', False))
    params.insert(sx.Float32Parameter(InterpolateGridDataFilter.FILL_VALUE, 'Fill Value', 'The value to use for points outside of the convex hull of the input points.  This option has no effect for the "Nearest" interpolation method.', 0.0))
    params.insert(sx.BoolParameter(InterpolateGridDataFilter.RESCALE_POINTS_TO_UNIT_CUBE, 'Rescale Points To Unit Cube', 'Rescale points to unit cube before performing interpolation. This is useful if some of the input dimensions have incommensurable units and differ by many orders of magnitude.', False))
    params.insert(sx.Parameters.Separator("Required Data Objects"))
    params.insert(sx.ArraySelectionParameter(InterpolateGridDataFilter.INPUT_POINTS_ARRAY_PATH, 'Input Points Array', 'This dataset contains coordinates where you have known values.', sx.DataPath(), {sx.DataType.float32, sx.DataType.float64}, [[2], [3]]))
    params.insert(sx.ArraySelectionParameter(InterpolateGridDataFilter.INPUT_DATA_ARRAY_PATH, 'Input Data Array', 'This dataset contains the measurements or data values corresponding to each point in your Input Points array.', sx.DataPath(), sx.get_all_data_types()))
    params.insert(sx.ArraySelectionParameter(InterpolateGridDataFilter.TARGET_POINTS_ARRAY_PATH, 'Target Points Array', 'This dataset contains coordinates where you want to estimate or predict the values based on your input dataset. These points can be, and often are, different from your input points.', sx.DataPath(), {sx.DataType.float32, sx.DataType.float64}, [[2], [3]]))
    params.insert(sx.Parameters.Separator("Created Data Objects"))
    params.insert(sx.ArrayCreationParameter(InterpolateGridDataFilter.INTERPOLATED_DATA_ARRAY_PATH, 'Interpolated Data Array', 'The output array of interpolated values corresponding to each of the target points specified in the Target Points array. This array provides the estimated values at the new points, based on the method of interpolation chosen and the Input Data array provided.', sx.DataPath("Interpolated Data")))
  ...
  def preflight_impl(self, data_structure: sx.DataStructure, args: dict, message_handler: sx.IFilter.MessageHandler, should_cancel: sx.AtomicBoolProxy) -> sx.IFilter.PreflightResult:
    ...
    # data_type: sx.DataType = from_numpy_dtype(input_data_array.dtype)
    # if input_data_array.data_type is None:
    #   return sx.IFilter.PreflightResult(sx.OutputActions(), [sx.Error(-1001, f"Array '{str(input_data_arr_path)}' has an unexpected data type ({input_data_array.dtype.name}).")])
  ...
  def execute_impl(self, data_structure: sx.DataStructure, args: dict, message_handler: sx.IFilter.MessageHandler, should_cancel: sx.AtomicBoolProxy) -> sx.IFilter.ExecuteResult:
    ...
    input_pts_array: sx.IDataArray = data_structure[input_pts_arr_path]
    ...
    interpolated_data_array: sx.IDataArray = data_structure[interpolated_data_arr_path]
    ...
    message_handler(sx.IFilter.Message(sx.IFilter.Message.Type.Info, f'Interpolating Grid Data...'))
