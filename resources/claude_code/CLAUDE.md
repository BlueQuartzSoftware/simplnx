...
Explain in clear terms what you want to do. Be precise when referring to classes, files or anything else. You can use the "Milestone" or "Phase" subheaders `### Milestone X` or `### Phase X` where you replace X with a number and step through the process that you want claude-code to perform. You can have claude do one thing at a time or you can have claude do the whole thing. Claude writes decent git commit messages so you can also leverage that ability. Be sure to scroll down to the configure and build sections to tell claude how to configure and then build on your system.
