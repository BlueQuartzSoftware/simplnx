...
TEST_CASE("ITKImageProcessing::ITKNotImage: defaults", "[ITKImageProcessing][ITKNotImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKNotImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKNotImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_NotImageFilter_defaults.nrrd";
