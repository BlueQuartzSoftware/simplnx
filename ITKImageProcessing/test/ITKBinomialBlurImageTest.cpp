...
TEST_CASE("ITKImageProcessing::ITKBinomialBlurImage: defaults", "[ITKImageProcessing][ITKBinomialBlurImage]")
{
  ...
  // Test the filter itself
  {
    ...
    auto pRepetitions = 1u;
    args.insertOrAssign(ITKBinomialBlurImage::k_Repetitions_Key, std::make_any<float64>(pRepetitions));
    args.insertOrAssign(ITKBinomialBlurImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKBinomialBlurImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKBinomialBlurImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_BinomialBlurImageFilter_defaults.nrrd";
