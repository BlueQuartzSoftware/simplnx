...
TEST_CASE("ITKImageProcessing::ITKBilateralImage: default", "[ITKImageProcessing][ITKBilateralImage]")
{
  ...
  // Test the filter itself
  {
    ...
    auto pDomainSigma = 4.0;
    auto pRangeSigma = 50.0;
    auto pNumberOfRangeGaussianSamples = 100u;
    args.insertOrAssign(ITKBilateralImage::k_DomainSigma_Key, std::make_any<float64>(pDomainSigma));
    args.insertOrAssign(ITKBilateralImage::k_RangeSigma_Key, std::make_any<float64>(pRangeSigma));
    args.insertOrAssign(ITKBilateralImage::k_NumberOfRangeGaussianSamples_Key, std::make_any<float64>(pNumberOfRangeGaussianSamples));
    args.insertOrAssign(ITKBilateralImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKBilateralImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKBilateralImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_BilateralImageFilter_default.nrrd";
...
// small kernel in 3d
TEST_CASE("ITKImageProcessing::ITKBilateralImage: 3d", "[ITKImageProcessing][ITKBilateralImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKBilateralImage filter;
  DataStructure ds;
  ...
  // Test the filter itself
  {
    ...
    DataPath inputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_InputDataPath);
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    auto pDomainSigma = 2.0;
    auto pRangeSigma = 500;
    auto pNumberOfRangeGaussianSamples = 100u;
    args.insertOrAssign(ITKBilateralImage::k_DomainSigma_Key, std::make_any<float64>(pDomainSigma));
    args.insertOrAssign(ITKBilateralImage::k_RangeSigma_Key, std::make_any<float64>(pRangeSigma));
    args.insertOrAssign(ITKBilateralImage::k_NumberOfRangeGaussianSamples_Key, std::make_any<float64>(pNumberOfRangeGaussianSamples));
