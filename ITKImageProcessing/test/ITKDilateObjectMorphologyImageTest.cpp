...
TEST_CASE("ITKImageProcessing::ITKDilateObjectMorphologyImage: float", "[ITKImageProcessing][ITKDilateObjectMorphologyImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKDilateObjectMorphologyImage::k_ObjectValue_Key, std::make_any<float64>(pObjectValue));
    args.insertOrAssign(ITKDilateObjectMorphologyImage::k_KernelRadius_Key, std::make_any<VectorFloat32Parameter::ValueType>(pKernelRadius));
    args.insertOrAssign(ITKDilateObjectMorphologyImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKDilateObjectMorphologyImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKDilateObjectMorphologyImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
...
TEST_CASE("ITKImageProcessing::ITKDilateObjectMorphologyImage: short", "[ITKImageProcessing][ITKDilateObjectMorphologyImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKDilateObjectMorphologyImage filter;
  DataStructure ds;
