...
TEST_CASE("ITKImageProcessing::ITKValuedRegionalMinimaImage: defaults", "[ITKImageProcessing][ITKValuedRegionalMinimaImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKValuedRegionalMinimaImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKValuedRegionalMinimaImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKValuedRegionalMinimaImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKValuedRegionalMinimaImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_ValuedRegionalMinimaImageFilter_defaults.nrrd";
