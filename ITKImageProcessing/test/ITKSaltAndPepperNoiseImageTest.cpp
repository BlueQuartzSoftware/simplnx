...
TEST_CASE("ITKSaltAndPepperNoiseImageFilter(2d)", "[.][ITKImageProcessing][ITKSaltAndPepperNoiseImage][2d][!mayfail]")
{
  DataStructure ds;
  ...
  Result<> compareResult = ITKTestBase::CompareImages(ds, baselineGeometryPath, baselineDataPath, inputGeometryPath, outputDataPath, 20);
  COMPLEX_RESULT_REQUIRE_VALID(compareResult);
}

TEST_CASE("ITKSaltAndPepperNoiseImageFilter(3d)", "[.][ITKImageProcessing][ITKSaltAndPepperNoiseImage][3d][!mayfail]")
{
  DataStructure ds;
  ITKSaltAndPepperNoiseImage filter;
  ...
  DataPath baselineDataPath = baselineGeometryPath.createChildPath(ITKTestBase::k_BaselineDataPath);
  Result<> readBaselineResult = ITKTestBase::ReadImage(ds, baselineFilePath, baselineGeometryPath, baselineDataPath);
  Result<> compareResult = ITKTestBase::CompareImages(ds, baselineGeometryPath, baselineDataPath, inputGeometryPath, outputDataPath, 10000.0);
  COMPLEX_RESULT_REQUIRE_VALID(compareResult);
}

TEST_CASE("ITKSaltAndPepperNoiseImageFilter(rgb)", "[.][ITKImageProcessing][ITKSaltAndPepperNoiseImage][rgb][!mayfail]")
{
  DataStructure ds;
  ITKSaltAndPepperNoiseImage filter;
