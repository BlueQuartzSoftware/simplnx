...
TEST_CASE("ITKImageProcessing::ITKSquareImage: defaults", "[ITKImageProcessing][ITKSquareImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKSquareImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKSquareImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_SquareImageFilter_defaults.nrrd";
