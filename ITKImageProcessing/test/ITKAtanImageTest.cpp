...
TEST_CASE("ITKImageProcessing::ITKAtanImage: defaults", "[ITKImageProcessing][ITKAtanImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKAtanImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKAtanImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
