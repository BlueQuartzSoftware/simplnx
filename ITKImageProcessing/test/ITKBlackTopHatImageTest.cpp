...
// Test black top hat
TEST_CASE("ITKImageProcessing::ITKBlackTopHatImage: BlackTopHapErode", "[ITKImageProcessing][ITKBlackTopHatImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKBlackTopHatImage::k_SafeBorder_Key, std::make_any<bool>(pSafeBorder));
    args.insertOrAssign(ITKBlackTopHatImage::k_KernelRadius_Key, std::make_any<VectorFloat32Parameter::ValueType>(pKernelRadius));
    args.insertOrAssign(ITKBlackTopHatImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKBlackTopHatImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKBlackTopHatImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_BlackTopHatImageFilter_BlackTopHapErode.nrrd";
