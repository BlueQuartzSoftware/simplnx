...
TEST_CASE("ITKImageProcessing::ITKBinaryClosingByReconstructionImage: BinaryClosingByReconstruction", "[ITKImageProcessing][ITKBinaryClosingByReconstructionImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKBinaryClosingByReconstructionImage::k_ForegroundValue_Key, std::make_any<float64>(pForegroundValue));
    args.insertOrAssign(ITKBinaryClosingByReconstructionImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKBinaryClosingByReconstructionImage::k_KernelRadius_Key, std::make_any<VectorFloat32Parameter::ValueType>(pKernelRadius));
    ...
    args.insertOrAssign(ITKBinaryClosingByReconstructionImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKBinaryClosingByReconstructionImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    fs::path baselineFilePath =
        fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_BinaryClosingByReconstructionImageFilter_BinaryClosingByReconstruction.nrrd";
