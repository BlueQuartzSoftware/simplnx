...
TEST_CASE("ITKImageProcessing::ITKClosingByReconstructionImage: ClosingByReconstruction", "[ITKImageProcessing][ITKClosingByReconstructionImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKClosingByReconstructionImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKClosingByReconstructionImage::k_PreserveIntensities_Key, std::make_any<bool>(pPreserveIntensities));
    args.insertOrAssign(ITKClosingByReconstructionImage::k_KernelRadius_Key, std::make_any<VectorFloat32Parameter::ValueType>(pKernelRadius));
    args.insertOrAssign(ITKClosingByReconstructionImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKClosingByReconstructionImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKClosingByReconstructionImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    fs::path baselineFilePath =
        fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_ClosingByReconstructionImageFilter_ClosingByReconstruction.nrrd";
