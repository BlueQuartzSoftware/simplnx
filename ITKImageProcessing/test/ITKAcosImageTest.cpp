...
TEST_CASE("ITKImageProcessing::ITKAcosImage: defaults", "[ITKImageProcessing][ITKAcosImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKAcosImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKAcosImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
