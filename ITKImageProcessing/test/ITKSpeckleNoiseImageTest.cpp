...
TEST_CASE("ITKImageProcessing::ITKSpeckleNoiseImage: 2d", "[ITKImageProcessing][ITKSpeckleNoiseImage]")
{
  ...
  // Test the filter itself
  {
    ...
    auto pSeed = 123;
    auto pStandardDeviation = 1.0;
    args.insertOrAssign(ITKSpeckleNoiseImage::k_StandardDeviation_Key, std::make_any<float64>(pStandardDeviation));
    args.insertOrAssign(ITKSpeckleNoiseImage::k_Seed_Key, std::make_any<float64>(pSeed));
    args.insertOrAssign(ITKSpeckleNoiseImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKSpeckleNoiseImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKSpeckleNoiseImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
...
TEST_CASE("ITKImageProcessing::ITKSpeckleNoiseImage: 3d", "[ITKImageProcessing][ITKSpeckleNoiseImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKSpeckleNoiseImage filter;
  DataStructure ds;
...
TEST_CASE("ITKImageProcessing::ITKSpeckleNoiseImage: rgb", "[ITKImageProcessing][ITKSpeckleNoiseImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKSpeckleNoiseImage filter;
  DataStructure ds;
