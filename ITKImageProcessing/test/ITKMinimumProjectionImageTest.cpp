...
TEST_CASE("ITKImageProcessing::ITKMinimumProjectionImage: defaults", "[ITKImageProcessing][ITKMinimumProjectionImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKMinimumProjectionImage::k_ProjectionDimension_Key, std::make_any<float64>(pProjectionDimension));
    args.insertOrAssign(ITKMinimumProjectionImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKMinimumProjectionImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKMinimumProjectionImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_MinimumProjectionImageFilter_defaults.nrrd";
...
TEST_CASE("ITKImageProcessing::ITKMinimumProjectionImage: another_dimension", "[ITKImageProcessing][ITKMinimumProjectionImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKMinimumProjectionImage filter;
  DataStructure ds;
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    int32_t error = complex::ITKTestBase::WriteImage(ds, filePath, inputGeometryPath, outputDataPath);
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_MinimumProjectionImageFilter_another_dimension.nrrd";
    DataPath baselineGeometryPath({complex::ITKTestBase::k_BaselineGeometryPath});
    DataPath baselineDataPath = baselineGeometryPath.createChildPath(complex::ITKTestBase::k_BaselineDataPath);
    // Compare md5 hash of final image
    std::string md5Hash = complex::ITKTestBase::ComputeMd5Hash(ds, outputDataPath);
...
TEST_CASE("ITKImageProcessing::ITKMinimumProjectionImage: short_image", "[ITKImageProcessing][ITKMinimumProjectionImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKMinimumProjectionImage filter;
  DataStructure ds;
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    int32_t error = complex::ITKTestBase::WriteImage(ds, filePath, inputGeometryPath, outputDataPath);
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_MinimumProjectionImageFilter_short_image.nrrd";
    DataPath baselineGeometryPath({complex::ITKTestBase::k_BaselineGeometryPath});
    DataPath baselineDataPath = baselineGeometryPath.createChildPath(complex::ITKTestBase::k_BaselineDataPath);
    // Compare md5 hash of final image
    std::string md5Hash = complex::ITKTestBase::ComputeMd5Hash(ds, outputDataPath);
...
TEST_CASE("ITKImageProcessing::ITKMinimumProjectionImage: rgb_image", "[ITKImageProcessing][ITKMinimumProjectionImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKMinimumProjectionImage filter;
  DataStructure ds;
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    int32_t error = complex::ITKTestBase::WriteImage(ds, filePath, inputGeometryPath, outputDataPath);
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_MinimumProjectionImageFilter_rgb_image.nrrd";
    DataPath baselineGeometryPath({complex::ITKTestBase::k_BaselineGeometryPath});
    DataPath baselineDataPath = baselineGeometryPath.createChildPath(complex::ITKTestBase::k_BaselineDataPath);
    // Compare md5 hash of final image
    std::string md5Hash = complex::ITKTestBase::ComputeMd5Hash(ds, outputDataPath);
