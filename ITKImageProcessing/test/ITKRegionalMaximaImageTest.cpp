...
TEST_CASE("ITKImageProcessing::ITKRegionalMaximaImage: defaults", "[ITKImageProcessing][ITKRegionalMaximaImage]")
{
  ...
  // Test the filter itself
  {
    ...
    auto pFlatIsMaxima = true;
    args.insertOrAssign(ITKRegionalMaximaImage::k_BackgroundValue_Key, std::make_any<float64>(pBackgroundValue));
    args.insertOrAssign(ITKRegionalMaximaImage::k_ForegroundValue_Key, std::make_any<float64>(pForegroundValue));
    args.insertOrAssign(ITKRegionalMaximaImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKRegionalMaximaImage::k_FlatIsMaxima_Key, std::make_any<bool>(pFlatIsMaxima));
    args.insertOrAssign(ITKRegionalMaximaImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKRegionalMaximaImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKRegionalMaximaImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_RegionalMaximaImageFilter_defaults.nrrd";
