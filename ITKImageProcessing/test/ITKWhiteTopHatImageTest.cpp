...
// Test white top hat
TEST_CASE("ITKImageProcessing::ITKWhiteTopHatImage: WhiteTopHatErode", "[ITKImageProcessing][ITKWhiteTopHatImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKWhiteTopHatImage::k_SafeBorder_Key, std::make_any<bool>(pSafeBorder));
    args.insertOrAssign(ITKWhiteTopHatImage::k_KernelRadius_Key, std::make_any<VectorFloat32Parameter::ValueType>(pKernelRadius));
    args.insertOrAssign(ITKWhiteTopHatImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKWhiteTopHatImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKWhiteTopHatImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_WhiteTopHatImageFilter_WhiteTopHatErode.nrrd";
