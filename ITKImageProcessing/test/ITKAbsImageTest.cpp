...
TEST_CASE("ITKImageProcessing::ITKAbsImage: float", "[ITKImageProcessing][ITKAbsImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKAbsImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKAbsImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
...
TEST_CASE("ITKImageProcessing::ITKAbsImage: short", "[ITKImageProcessing][ITKAbsImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKAbsImage filter;
  DataStructure ds;
