...
TEST_CASE("ITKImageProcessing::ITKOpeningByReconstructionImage: OpeningByReconstruction", "[ITKImageProcessing][ITKOpeningByReconstructionImage]")
{
  ...
  // Test the filter itself
  {
    ...
    auto pPreserveIntensities = false;
    args.insertOrAssign(ITKOpeningByReconstructionImage::k_KernelType_Key, std::make_any<ChoicesParameter::ValueType>(pKernelType));
    args.insertOrAssign(ITKOpeningByReconstructionImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKOpeningByReconstructionImage::k_PreserveIntensities_Key, std::make_any<bool>(pPreserveIntensities));
    args.insertOrAssign(ITKOpeningByReconstructionImage::k_KernelRadius_Key, std::make_any<VectorFloat32Parameter::ValueType>(pKernelRadius));
    args.insertOrAssign(ITKOpeningByReconstructionImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKOpeningByReconstructionImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKOpeningByReconstructionImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    fs::path baselineFilePath =
        fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_OpeningByReconstructionImageFilter_OpeningByReconstruction.nrrd";
