...
TEST_CASE("ITKImageProcessing::ITKSumProjectionImage: z_projection", "[ITKImageProcessing][ITKSumProjectionImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKSumProjectionImage::k_ProjectionDimension_Key, std::make_any<float64>(pProjectionDimension));
    args.insertOrAssign(ITKSumProjectionImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKSumProjectionImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKSumProjectionImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
