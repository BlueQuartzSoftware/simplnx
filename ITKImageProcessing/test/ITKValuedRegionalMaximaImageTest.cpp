...
TEST_CASE("ITKImageProcessing::ITKValuedRegionalMaximaImage: defaults", "[ITKImageProcessing][ITKValuedRegionalMaximaImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKValuedRegionalMaximaImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKValuedRegionalMaximaImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKValuedRegionalMaximaImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKValuedRegionalMaximaImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_ValuedRegionalMaximaImageFilter_defaults.nrrd";
