...
TEST_CASE("ITKImageProcessing::ITKLog10Image: defaults", "[ITKImageProcessing][ITKLog10Image]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKLog10Image::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKLog10Image::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
