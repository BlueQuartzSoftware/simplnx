...
TEST_CASE("ITKImageProcessing::ITKGrayscaleGrindPeakImage: GrayscaleGrindPeak1", "[ITKImageProcessing][ITKGrayscaleGrindPeakImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKGrayscaleGrindPeakImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKGrayscaleGrindPeakImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKGrayscaleGrindPeakImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKGrayscaleGrindPeakImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    fs::path baselineFilePath =
        fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_GrayscaleGrindPeakImageFilter_GrayscaleGrindPeak1.nrrd";
...
TEST_CASE("ITKImageProcessing::ITKGrayscaleGrindPeakImage: GrayscaleGrindPeak2", "[ITKImageProcessing][ITKGrayscaleGrindPeakImage]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKGrayscaleGrindPeakImage filter;
  DataStructure ds;
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath =
        fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_GrayscaleGrindPeakImageFilter_GrayscaleGrindPeak2.nrrd";
    DataPath baselineGeometryPath({complex::ITKTestBase::k_BaselineGeometryPath});
    DataPath baselineDataPath = baselineGeometryPath.createChildPath(complex::ITKTestBase::k_BaselineDataPath);
    // Compare md5 hash of final image
    std::string md5Hash = complex::ITKTestBase::ComputeMd5Hash(ds, outputDataPath);
