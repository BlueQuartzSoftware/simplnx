...
// Test HMinima
TEST_CASE("ITKImageProcessing::ITKHMinimaImage: HMinima", "[ITKImageProcessing][ITKHMinimaImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKHMinimaImage::k_Height_Key, std::make_any<float64>(pHeight));
    args.insertOrAssign(ITKHMinimaImage::k_FullyConnected_Key, std::make_any<bool>(pFullyConnected));
    args.insertOrAssign(ITKHMinimaImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKHMinimaImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKHMinimaImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_HMinimaImageFilter_HMinima.nrrd";
