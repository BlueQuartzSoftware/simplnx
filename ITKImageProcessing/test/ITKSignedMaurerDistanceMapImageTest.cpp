...
TEST_CASE("ITKImageProcessing::ITKSignedMaurerDistanceMapImage: default", "[ITKImageProcessing][ITKSignedMaurerDistanceMapImage]")
{
  ...
  // Test the filter itself
  {
    ...
    auto pInsideIsPositive = false;
    auto pSquaredDistance = true;
    auto pUseImageSpacing = false;
    auto pBackgroundValue = 0.0;
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_InsideIsPositive_Key, std::make_any<bool>(pInsideIsPositive));
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_SquaredDistance_Key, std::make_any<bool>(pSquaredDistance));
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_UseImageSpacing_Key, std::make_any<bool>(pUseImageSpacing));
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_BackgroundValue_Key, std::make_any<float64>(pBackgroundValue));
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
