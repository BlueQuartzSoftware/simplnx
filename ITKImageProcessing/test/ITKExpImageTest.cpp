...
TEST_CASE("ITKImageProcessing::ITKExpImage: defaults", "[ITKImageProcessing][ITKExpImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKExpImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKExpImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
