...
TEST_CASE("ITKImageProcessing::ITKSigmoidImage: defaults", "[ITKImageProcessing][ITKSigmoidImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKSigmoidImage::k_Alpha_Key, std::make_any<float64>(pAlpha));
    args.insertOrAssign(ITKSigmoidImage::k_Beta_Key, std::make_any<float64>(pBeta));
    args.insertOrAssign(ITKSigmoidImage::k_OutputMaximum_Key, std::make_any<float64>(pOutputMaximum));
    args.insertOrAssign(ITKSigmoidImage::k_OutputMinimum_Key, std::make_any<float64>(pOutputMinimum));
    args.insertOrAssign(ITKSigmoidImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
    args.insertOrAssign(ITKSigmoidImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKSigmoidImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_SigmoidImageFilter_defaults.nrrd";
