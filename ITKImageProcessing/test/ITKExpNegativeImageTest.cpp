...
TEST_CASE("ITKImageProcessing::ITKExpNegativeImage: defaults", "[ITKImageProcessing][ITKExpNegativeImage]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKExpNegativeImage::k_SelectedCellArrayPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKExpNegativeImage::k_NewCellArrayName_Key, std::make_any<DataPath>(outputDataPath));
