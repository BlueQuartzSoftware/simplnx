...
TEST_CASE("ITKCurvatureAnisotropicDiffusionImageFilter(defaults)", "[ITKImageProcessing][ITKCurvatureAnisotropicDiffusionImage][defaults]")
{
  ...
  fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.view() / "JSONFilters/Baseline/BasicFilters_CurvatureAnisotropicDiffusionImageFilter_defaults.nrrd";
  ...
  Result<> compareResult = ITKTestBase::CompareImages(ds, baselineGeometryPath, baselineDataPath, inputGeometryPath, outputDataPath, 0.1);
...
TEST_CASE("ITKCurvatureAnisotropicDiffusionImageFilter(longer)", "[ITKImageProcessing][ITKCurvatureAnisotropicDiffusionImage][longer]")
{
  ...
  fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.view() / "JSONFilters/Baseline/BasicFilters_CurvatureAnisotropicDiffusionImageFilter_longer.nrrd";
  DataPath baselineGeometryPath({ITKTestBase::k_BaselineGeometryPath});
  DataPath baseLineCellDataPath = baselineGeometryPath.createChildPath(ITKTestBase::k_ImageCellDataPath);
  DataPath baselineDataPath = baseLineCellDataPath.createChildPath(ITKTestBase::k_BaselineDataPath);
  Result<> readBaselineResult = ITKTestBase::ReadImage(ds, baselineFilePath, baselineGeometryPath, baseLineCellDataPath, baselineDataPath);
