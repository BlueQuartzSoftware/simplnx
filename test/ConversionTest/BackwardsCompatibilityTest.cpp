...
void CreateMapInput(Uuid&& uuid, ArrayThresholdsParameter::ValueType&& value)
{
  ...
  else
  {
    k_ParamMap.emplace(uuid, std::make_pair(std::vector<std::any>{std::make_any<ArrayThresholdsParameter::ValueType>(value)}, [](const std::any& imported, const std::any& exemplar) -> bool {
                         auto importedRef = static_cast<ArrayThreshold*>(GetAnyRef<ArrayThresholdsParameter::ValueType>(imported).getArrayThresholds().at(0).get());
                         auto exemplarRef = static_cast<ArrayThreshold*>(GetAnyRef<ArrayThresholdsParameter::ValueType>(exemplar).getArrayThresholds().at(0).get());
...
TEST_CASE("nx::core::Test Filter Parameter Conversion", "[simplnx][Filter]")
{
  ...
  std::vector<std::string> noConversionStrings = {};
  ...
  for(int32 i = 0; i < numFilters; i++)
  {
    ...
    if(!simplData.has_value())
    {
      ...
      if(filterJson.contains(k_SIMPLFilterHumanNameKey))
      {
        filterName = fmt::format("{} with uuid {}", filterJson[k_SIMPLFilterHumanNameKey].get<std::string>(), uuidString);
      }
      noConversionStrings.emplace_back(std::move(filterName));
      continue;
    }
