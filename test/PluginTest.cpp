...
TEST_CASE("Dump Python Named Arguments")
{
  ...
  for(const auto& filterHandle : filterHandles)
  {
    ...
    std::ofstream docStream = std::ofstream(docFilePath, std::ios_base::binary | std::ios_base::app);
    ...
    if(plugin->getName() == "ComplexCore")
    {
      docStream << "+ module: complex\n";
    }
    else
    {
      docStream << "+ module: " << plugin->getName() << "\n";
    }
    docStream << "+ Class Name: " << filterHandle.getClassName() << "\n";
    docStream << "+ Displayed Name: " << filterHandle.getFilterName() << "\n\n";
