...
TEST_CASE("DataStructure resolver: per-instance policies remain isolated and copy", "[Core][FormatResolver]")
{
  DataStructure a;
  DataStructure b;
  a.setFormatResolver(std::make_shared<FixedResolver>("PER-A"));
  b.setFormatResolver(std::make_shared<FixedResolver>("PER-B"));

  const DataPath p({"x"});
  REQUIRE(a.formatResolver().resolveFormat(a, p, DataType::int32, 0) == "PER-A");
