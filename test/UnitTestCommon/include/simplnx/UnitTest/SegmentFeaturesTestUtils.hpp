    ...
    for(usize y = 0; y < dimY; y++)
    {
      for(usize x = 0; x < dimX; x++)
      {
        ...
        if(wrapBoundary)
        {
          ...
          store[idx] = static_cast<int32>(wbz * wbpy * wbpx + wby * wbpx + wbx);
        }
        else
        {
          store[idx] = static_cast<int32>(bz * blocksPerY * blocksPerX + by * blocksPerX + bx);
        }
      }
    }
  }
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    for(usize y = 0; y < dimY; y++)
    {
      for(usize x = 0; x < dimX; x++)
      {
        ...
        quatsStore[idx * 4 + 0] = q[0];
        quatsStore[idx * 4 + 1] = q[1];
        quatsStore[idx * 4 + 2] = q[2];
        quatsStore[idx * 4 + 3] = q[3];
      }
    }
  }
...
inline void BuildSphericalMask(DataStructure& ds, const ShapeType& cellShape, DataObject::IdType amId, const std::string& maskName = "Mask")
{
  ...
  const float cx = static_cast<float>(dimX) / 2.0f;
  const float cy = static_cast<float>(dimY) / 2.0f;
  const float cz = static_cast<float>(dimZ) / 2.0f;
  const float radius = std::min({cx, cy, cz}) * 0.8f;
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    for(usize y = 0; y < dimY; y++)
    {
      for(usize x = 0; x < dimX; x++)
      {
        ...
        maskStore[idx] = (dx * dx + dy * dy + dz * dz < radius * radius) ? 1 : 0;
      }
    }
  }
}
