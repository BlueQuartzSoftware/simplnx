...
inline void BuildScalarTestData(DataStructure& ds, const ShapeType& cellShape, DataObject::IdType amId, usize blockSize, const std::string& arrayName = "ScalarData", bool wrapBoundary = false)
{
  ...
  auto scalarDataStore = DataStoreUtilities::CreateDataStore<int32>(ds, scalarPath, cellShape, {1});
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    ...
    auto storeWriteResult = store.copyFromBuffer(z * sliceSize, nonstd::span<const int32>(sliceBuffer.data(), sliceSize));
                                     ...
                                     bool wrapBoundary = false)
{
  ...
  auto quatsDataStore = DataStoreUtilities::CreateDataStore<float32>(ds, quatsPath, cellShape, {4});
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    ...
    auto quatsStoreWriteResult = quatsStore.copyFromBuffer(z * sliceSize * 4, nonstd::span<const float32>(quatsSliceBuffer.data(), sliceSize * 4));
    SIMPLNX_RESULT_REQUIRE_VALID(quatsStoreWriteResult);
  ...
  auto crystalDataStore = DataStoreUtilities::CreateDataStore<uint32>(ds, crystalStructsPath, ensembleTupleShape, {1});
...
inline void BuildSphericalMask(DataStructure& ds, const ShapeType& cellShape, DataObject::IdType amId, const std::string& maskName = "Mask")
{
  ...
  auto maskDataStore = DataStoreUtilities::CreateDataStore<uint8>(ds, maskPath, cellShape, {1});
  ...
  for(usize z = 0; z < dimZ; z++)
  {
    ...
    auto maskStoreWriteResult = maskStore.copyFromBuffer(z * sliceSize, nonstd::span<const uint8>(sliceBuffer.data(), sliceSize));
 ...
 * @param algorithmTestScope Optional scope that proves the selector
...
void RunNoValidVoxelsErrorTest(AlgorithmTestScope* algorithmTestScope, SetupArgsFn setupArgs)
{
  ...
  auto executeResult = algorithmTestScope == nullptr ? filter.execute(ds, args) : algorithmTestScope->executeFilter(filter, ds, args);
 ...
 * @brief Verifies that a SegmentFeatures filter returns error -87000 for an empty mask.
 * @tparam FilterT Specifies the SegmentFeatures filter type.
 * @tparam SetupArgsFn Specifies the
 * argument-setup callable type.
 * @param setupArgs Adds filter-specific arguments for the test paths.
 */
template <typename FilterT, typename SetupArgsFn>
void RunNoValidVoxelsErrorTest(SetupArgsFn setupArgs)
{
  RunNoValidVoxelsErrorTest<FilterT>(nullptr, std::move(setupArgs));
}
 ...
 * @tparam SetupArgsFn Specifies the
 * argument-setup callable type.
 * @param algorithmTestScope Proves the selector for the target execution.
 * @param setupArgs Adds filter-specific arguments for the test paths.
 */
template <typename FilterT, typename SetupArgsFn>
void RunNoValidVoxelsErrorTest(AlgorithmTestScope& algorithmTestScope, SetupArgsFn setupArgs)
{
  RunNoValidVoxelsErrorTest<FilterT>(&algorithmTestScope, std::move(setupArgs));
}
 ...
 * @tparam SetupFevFn Specifies the FaceEdgeVertex setup
 * callable type.
 * @param algorithmTestScope Optional scope that proves the selector for each target execution.
 * @param setupFaceArgs Adds filter arguments with face connectivity.
 * @param
 * setupFevArgs Adds filter arguments with FaceEdgeVertex connectivity.
 */
template <typename FilterT, typename SetupFaceFn, typename SetupFevFn>
void RunFaceEdgeVertexConnectivityTest(AlgorithmTestScope* algorithmTestScope, SetupFaceFn setupFaceArgs, SetupFevFn setupFevArgs)
{
  constexpr usize kDim = 3;
  const std::array<usize, 3> dims = {kDim, kDim, kDim};
  const ShapeType cellShape = {kDim, kDim, kDim};
 ...
 * @brief Verifies FaceEdgeVertex connectivity for vertex-connected and edge-connected regions.
 * @tparam FilterT Specifies the SegmentFeatures filter type.
 * @tparam SetupFaceFn Specifies the
 * face-connectivity setup callable type.
...
{
  RunFaceEdgeVertexConnectivityTest<FilterT>(nullptr, std::move(setupFaceArgs), std::move(setupFevArgs));
}
 ...
 * face-connectivity setup callable type.
 * @tparam SetupFevFn Specifies the FaceEdgeVertex setup callable type.
 * @param algorithmTestScope Proves the selector for each target execution.
 * @param
 * setupFaceArgs Adds filter arguments with face connectivity.
 * @param setupFevArgs Adds filter arguments with FaceEdgeVertex connectivity.
 */
template <typename FilterT, typename SetupFaceFn, typename SetupFevFn>
void RunFaceEdgeVertexConnectivityTest(AlgorithmTestScope& algorithmTestScope, SetupFaceFn setupFaceArgs, SetupFevFn setupFevArgs)
{
  RunFaceEdgeVertexConnectivityTest<FilterT>(&algorithmTestScope, std::move(setupFaceArgs), std::move(setupFevArgs));
}
 ...
 */
template <typename FilterT, typename SetupArgsFn>
void RunNeighborSchemeExemplarTest(AlgorithmTestScope* algorithmTestScope, DataStructure& dataStructure, const DataPath& computedFeatureIdsPath, const DataPath& activesPath,
                                   const DataPath& exemplarFeatureIdsPath, usize expectedFeatureCount, SetupArgsFn setupArgs, const std::vector<DataPath>& tupleCheckIgnoredPaths = {})
{
  FilterT filter;
  Arguments args;
  setupArgs(args);
  ...
  auto executeResult = algorithmTestScope == nullptr ? filter.execute(dataStructure, args) : algorithmTestScope->executeFilter(filter, dataStructure, args);
  SIMPLNX_RESULT_REQUIRE_VALID(executeResult.result);
 ...
 * @brief Executes a SegmentFeatures filter and verifies its exemplar output.
 * @tparam FilterT Specifies the SegmentFeatures filter type.
 * @tparam SetupArgsFn Specifies the argument-setup
 * callable type.
 * @param dataStructure Contains the exemplar input and receives computed output.
 * @param computedFeatureIdsPath Computed FeatureIds array path.
 * @param activesPath Computed
 * Active array path.
 * @param exemplarFeatureIdsPath Exemplar FeatureIds array path.
 * @param expectedFeatureCount Expected Active tuple count, or 0 to omit this check.
 * @param setupArgs Adds the
...
{
  RunNeighborSchemeExemplarTest<FilterT>(nullptr, dataStructure, computedFeatureIdsPath, activesPath, exemplarFeatureIdsPath, expectedFeatureCount, std::move(setupArgs), tupleCheckIgnoredPaths);
}
 ...
 * @param dataStructure Contains the exemplar input and receives computed output.
 * @param
 * computedFeatureIdsPath Computed FeatureIds array path.
 * @param activesPath Computed Active array path.
 * @param exemplarFeatureIdsPath Exemplar FeatureIds array path.
 * @param
 * expectedFeatureCount Expected Active tuple count, or 0 to omit this check.
 * @param setupArgs Adds the filter-specific arguments.
 * @param tupleCheckIgnoredPaths Paths omitted from the
 * tuple-dimension check.
 */
template <typename FilterT, typename SetupArgsFn>
void RunNeighborSchemeExemplarTest(AlgorithmTestScope& algorithmTestScope, DataStructure& dataStructure, const DataPath& computedFeatureIdsPath, const DataPath& activesPath,
                                   const DataPath& exemplarFeatureIdsPath, usize expectedFeatureCount, SetupArgsFn setupArgs, const std::vector<DataPath>& tupleCheckIgnoredPaths = {})
{
  RunNeighborSchemeExemplarTest<FilterT>(&algorithmTestScope, dataStructure, computedFeatureIdsPath, activesPath, exemplarFeatureIdsPath, expectedFeatureCount, std::move(setupArgs),
                                         tupleCheckIgnoredPaths);
}
